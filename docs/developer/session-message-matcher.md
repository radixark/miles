...
> **状态：** 设计已接受并在本分支实现；定向回归与独立代码审查已通过，待合入。本文记录稳定契约、安全边界和验证要求。
>
...
实现不能只修改 session 层消息匹配器，否则 TITO 验证仍可能拒绝 session 层已经接受的前缀。本分支在纯规划阶段只确定一次 `effective_messages`，并让 TITO、代理请求、主记录和提交状态统一使用该结果。
...
冻结基线已通过聚焦的现有消息匹配器与 trajectory suite：`73 passed, 28 warnings`。实现分支已通过上述完整定向回归：`660 passed, 30 warnings`。
...
本设计文档不加入已发布的开发者导航。稳定用户契约已记录在 `docs/user-guide/agentic-chat-template.md` 和 `docs/user-guide/customization.md`；本文件继续作为设计理由与安全记录。
