...
from miles.utils.workers.worker_provider.kubernetes.helm import labels as helm_labels
from miles.utils.workers.worker_provider.kubernetes.helm.labels import DEFAULT_LABEL_KEYS
from miles.utils.workers.worker_provider.kubernetes.views import pod_info
...
def make_pod(
    ...
    **kwargs,
    ...
    pod_labels = {
        helm_labels.POOL_LABEL: pool_id,
        helm_labels.LWS_GROUP_INDEX_LABEL: cell_index,
        helm_labels.LWS_WORKER_INDEX_LABEL: pod_index,
    }
...
def parse(pod, keys=None):
    return pod_info.parse_pod(pod, keys or DEFAULT_LABEL_KEYS)


class TestParsePod:
    def test_reads_the_resource_ordinal_the_platform_assigned_the_cell(self):
        """A pool_id partitions its gpus and ports between its cells, and only the platform knows the split."""
        assert parse(make_pod(cell_index="3")).cell_ordinal == 3

    def test_reads_the_cell_a_pod_belongs_to(self):
        """A cell is a pool_id and a group index, which is what its consumers address."""
        parsed = parse(make_pod(pool_id="inference-engine-0-0", cell_index="2"))
    ...
    def test_ignores_a_pod_that_names_no_pool(self):
        ...
        pod = make_unlabelled_pod("engine-0-0", labels={helm_labels.LWS_GROUP_INDEX_LABEL: "0"})
    ...
    def test_reads_the_node_a_pod_landed_on(self):
        ...
        assert parse(make_pod()).node_name == "gpu-1"
    ...
    def test_reads_the_keys_a_platform_configured(self):
        ...
        keys = DEFAULT_LABEL_KEYS.model_copy(update={"pool_id": "acme.io/group", "cell_ordinal": "acme.io/index"})
    ...
    def test_reads_how_many_pods_the_cell_should_have(self):
        ...
        pod = make_pod(labels={helm_labels.LWS_SIZE_LABEL: "4"})
    ...
    def test_reads_the_domain_facts_a_platform_attached(self):
        ...
        pod = make_pod(annotations={f"{helm_labels.META_ANNOTATION_PREFIX}model_id": "glm", "other": "ignored"})
    ...
    def test_reads_the_gpus_the_platform_assigned(self):
        """A rank owns a share of its pod's gpus, which only the platform's annotation can say."""
        pod = make_pod(annotations={f"{helm_labels.META_ANNOTATION_PREFIX}{helm_labels.GPU_IDS_META}": "0,1,2,3"})
...
class TestParsePods:
    def test_returns_the_pods_of_a_cell_in_index_order(self):
        """Everything downstream numbers ranks off this order, so the store's order must not leak through."""
        raw = [make_pod(name=f"engine-0-{index}", pod_index=str(index)) for index in (2, 0, 1)]

        assert [pod.pod_index for pod in pod_info.parse_pods(raw, keys=DEFAULT_LABEL_KEYS)] == [0, 1, 2]
    ...
    def test_drops_the_pods_that_are_not_workers(self):
        ...
        assert len(pod_info.parse_pods(raw, keys=DEFAULT_LABEL_KEYS)) == 1
...
class TestCellMembersHash:
    def test_is_stable_for_the_same_membership(self):
        ...
        pods = [parse(make_pod(name=f"engine-0-{index}", pod_index=str(index))) for index in range(2)]

        assert pod_info.cell_members_hash(pods) == pod_info.cell_members_hash(list(reversed(pods)))
    ...
    def test_changes_when_a_pod_is_replaced(self):
        ...
        assert pod_info.cell_members_hash(before) != pod_info.cell_members_hash(after)
