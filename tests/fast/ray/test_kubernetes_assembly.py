...
STATIC_HOSTS = {
    pool_id: static_worker_host(RELEASE, pool_id, 0)
...
def install(monkeypatch: pytest.MonkeyPatch, *, pods: list[Pod], ranks_per_pod: int = 1):
    fake_pod_api.reset()
    ...
    specs = [
        trainer_spec(num_workers_per_cell=len(pods) * ranks_per_pod, num_gpus_per_node=ranks_per_pod),
