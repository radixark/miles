...
_TOPK_MULTIPLE = {"tilelang": 64, "flash_mla": 128}
_FLASH_MLA_HEADS = (64, 128)


def _pad_topk(indices, multiple: int):
...
def default_forward_backend(q, kv, d_v: int) -> str:
    if flash_mla_sparse_fwd is None or d_v != 512 or kv.shape[2] != 1 or q.shape[2] > _FLASH_MLA_HEADS[-1]:
        return "tilelang"
    return "flash_mla"
...
class _SparseAttention(torch.autograd.Function):
    ...
    def forward(ctx, q, kv, indices, attn_sink, sm_scale, d_v, backend):
        q, kv, indices = q.contiguous(), kv.contiguous(), _pad_topk(indices, _TOPK_MULTIPLE[backend])
...
def sparse_attention(
    ...
    ``forward_backend`` (``"flash_mla"`` / ``"tilelang"``) overrides :func:`default_forward_backend`."""
    if d_v is None:
        d_v = q.shape[-1]
    backend = forward_backend or default_forward_backend(q, kv, d_v)
