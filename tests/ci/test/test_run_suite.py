...
class TestResolvePolicy:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (REGULAR_CADENCE, {"run-ci-fsdp-colocated"}, {"fsdp-colocated"}, False),
            (REGULAR_CADENCE, {"bypass-fastfail"}, set(), True),
            (
                REGULAR_CADENCE,
                {"run-ci-image"},
                _ALL - {"long", "ft-short", "ft-long", "fsdp-colocated"},
                False,
            ),
            (REGULAR_CADENCE, {"run-ci-all"}, _ALL, False),
            (REGULAR_CADENCE, {"run-ci-image", "run-ci-all"}, _ALL, False),
            (NIGHTLY_CADENCE, set(), _ALL - {"long", "ft-long", "fsdp-colocated"}, True),
            (NIGHTLY_CADENCE, {"nightly"}, _ALL - {"long", "ft-long", "fsdp-colocated"}, True),
            (
                NIGHTLY_CADENCE,
                {"run-ci-image", "nightly"},
                _ALL - {"long", "ft-long", "fsdp-colocated"},
                True,
            ),
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                REGULAR_CADENCE,
                {"run-ci-image", "run-ci-long"},
                _ALL - {"ft-short", "ft-long", "fsdp-colocated"},
            ),
            (
                REGULAR_CADENCE,
                {"run-ci-image", "run-ci-ft-short"},
                _ALL - {"long", "ft-long", "fsdp-colocated"},
            ),
            (NIGHTLY_CADENCE, {"nightly", "run-ci-long"}, _ALL - {"ft-long", "fsdp-colocated"}),
            (NIGHTLY_CADENCE, {"nightly", "run-ci-ft-long"}, _ALL - {"long", "fsdp-colocated"}),
            (
                REGULAR_CADENCE,
                {"run-ci-image", "run-ci-ft-short", "run-ci-ft-long"},
                _ALL - {"long", "fsdp-colocated"},
            ),
            (NIGHTLY_CADENCE, {"run-ci-ft-long"}, _ALL - {"long", "fsdp-colocated"}),
            (
                REGULAR_CADENCE,
                {"run-ci-image", "run-ci-fsdp-colocated"},
                _ALL - {"long", "ft-short", "ft-long"},
            ),
            (
                NIGHTLY_CADENCE,
                {"nightly", "run-ci-fsdp-colocated"},
                _ALL - {"long", "ft-long"},
            ),
        ],
    )
...
def broad_scope_tests():
    return [
        ...
        _make("tests/e2e/fsdp_colocated.py", labels=["long", "fsdp-colocated"]),
...
class TestFilterTestsBroadScopes:
    ...
    def test_explicit_long_selector_selects_its_test(self, broad_scope_tests):
        enabled, _ = filter_tests(
            broad_scope_tests,
            HWBackend.CUDA,
            "stage-c-8-gpu-h100",
            labels=set(resolve_policy(REGULAR_CADENCE, {"run-ci-fsdp-colocated"}).include_labels),
        )
        assert _names(enabled) == {
            "tests/e2e/always.py",
            "tests/e2e/fsdp_colocated.py",
        }
