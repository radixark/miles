...
#define TMEM_ACC_DV_OFFSET 32
#define TMEM_ACC_P_OFFSET 64
...
#define SMEM_DO_PANEL_STAGE_BYTES 4096
#define SMEM_DO_PANEL_STRIDE 4096
...
#define SMEM_DO_MN_STAGE_BYTES 4096
#define SMEM_DO_MN_STRIDE 4096
#define SMEM_DHS_OFF 107520
#define SMEM_DHS_STAGE_BYTES 8192
#define SMEM_DHS_STRIDE 8192
#define SMEM_DHS_MN_OFF 107520
#define SMEM_DHS_MN_STAGE_BYTES 8192
#define SMEM_DHS_MN_STRIDE 8192
#define SMEM_BDV_OFF 115712
#define SMEM_BDV_STAGE_BYTES 4096
#define SMEM_BDV_STRIDE 4096
#define SMEM_BDV_MN_OFF 115712
#define SMEM_BDV_MN_STAGE_BYTES 4096
#define SMEM_BDV_MN_STRIDE 4096
...
kernel_cake_kda_chunk_train_f36b958b2e98912bca25(CakeTensorMap const* kg_tma, CakeTensorMap const* qg_tma, CakeTensorMap const* w_tma, CakeTensorMap const* do_tma, __nv_bfloat16* __restrict__ dv1, float* __restrict__ gk, __nv_bfloat16* __restrict__ dh_out, __nv_bfloat16* __restrict__ dv2, int num_heads, int* __restrict__ seq_chunk_start, float scale)
{
    ...
    __nv_bfloat16* dhs = reinterpret_cast<__nv_bfloat16*>(smem_raw + 107520);
    const int dhs_addr = smem + 107520;
    __nv_bfloat16* dhs_mn = reinterpret_cast<__nv_bfloat16*>(smem_raw + 107520);
    const int dhs_mn_addr = smem + 107520;
    __nv_bfloat16* bdv = reinterpret_cast<__nv_bfloat16*>(smem_raw + 115712);
    const int bdv_addr = smem + 115712;
    __nv_bfloat16* bdv_mn = reinterpret_cast<__nv_bfloat16*>(smem_raw + 115712);
    const int bdv_mn_addr = smem + 115712;
    ...
    const int tmem_acc_dv = taddr + 32;
    const int tmem_acc_p = taddr + 64;

    // ---- Role: compute ----
    if (warp <= 3) {
        { // compute_main
            int half = blockIdx.x % 4;
            int head = blockIdx.x / 4 % num_heads;
            int seq = blockIdx.x / (4 * num_heads);
            int c0 = seq_chunk_start[seq];
            int nc = seq_chunk_start[seq + 1] - c0;
            int row0 = c0 * 64;
            int vbase = half * 32;
            ...
            for (int rc = 0; rc < nc; rc++) {
                int c = nc - 1 - rc;
                int dh_base = (((c0 + c) * num_heads + head) * 128 + k_row) * 128 + vbase;
                #pragma unroll
                for (int blk_1 = 0; blk_1 < 2; blk_1++) {
                    ...
                    #pragma unroll
                    for (int q = 0; q < 2; q++) {
                        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((dhs_addr + (unsigned int)((blk_1 * 16 + q * 8) / 32 * 8192 + k_row * 64 + (blk_1 * 16 + q * 8) % 32 * 2 ^ ((blk_1 * 16 + q * 8) / 32 * 8192 + k_row * 64 + (blk_1 * 16 + q * 8) % 32 * 2 >> 7 & 3) << 4))), "r"(_tmem_load_0_bf16[q * 4]), "r"(_tmem_load_0_bf16[q * 4 + 1]), "r"(_tmem_load_0_bf16[q * 4 + 2]), "r"(_tmem_load_0_bf16[q * 4 + 3]) : "memory");
                    }
                }
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                ...
                for (int blk_2 = 0; blk_2 < 1; blk_2++) {
                    ...
                    asm volatile(
                        ...
                        : "r"(taddr + 32 + (unsigned int)m64_row_base + (unsigned int)(blk_2 * 32)));
                    asm volatile("tcgen05.wait::ld.sync.aligned;");
                    ...
                    for (int q_1 = 0; q_1 < 2; q_1++) {
                        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((bdv_addr + (unsigned int)((col + q_1 * 8) / 32 * 4096 + t_row * 64 + (col + q_1 * 8) % 32 * 2 ^ ((col + q_1 * 8) / 32 * 4096 + t_row * 64 + (col + q_1 * 8) % 32 * 2 >> 7 & 3) << 4))), "r"(bdvv_bf16[q_1 * 4]), "r"(bdvv_bf16[q_1 * 4 + 1]), "r"(bdvv_bf16[q_1 * 4 + 2]), "r"(bdvv_bf16[q_1 * 4 + 3]) : "memory");
                    }
                }
                ...
                asm volatile("tcgen05.fence::after_thread_sync;");
                #pragma unroll
                for (int blk_4 = 0; blk_4 < 2; blk_4++) {
                    float _tmem_load_3[16];
                    tmem_ld_x16(&_tmem_load_3[0], taddr + (unsigned int)m128_row_base + (unsigned int)(blk_4 * 16));
                    asm volatile("tcgen05.wait::ld.sync.aligned;");
                    float _tmem_load_4[16];
                    tmem_ld_x16(&_tmem_load_4[0], taddr + 64 + (unsigned int)m128_row_base + (unsigned int)(blk_4 * 16));
                    asm volatile("tcgen05.wait::ld.sync.aligned;");
                    float upd[16];
                    #pragma unroll
    ...
    if (warp == 4) {
        { // load_main
            int half_1 = blockIdx.x % 4;
            int head_1 = blockIdx.x / 4 % num_heads;
            int seq_1 = blockIdx.x / (4 * num_heads);
            int c0_1 = seq_chunk_start[seq_1];
            int nc_1 = seq_chunk_start[seq_1 + 1] - c0_1;
            int row0_1 = c0_1 * 64;
            unsigned int stage = 0;
            unsigned int _phase_tiles_empty = 1;
            if (elect_sync()) {
                #pragma unroll 1
                for (int rc_1 = 0; rc_1 < nc_1; rc_1++) {
                    int c_1 = nc_1 - 1 - rc_1;
                    mbarrier_wait(tiles_empty_addr + (stage) * 8, _phase_tiles_empty);
                    mbarrier_arrive_expect_tx(tiles_full_addr + (stage) * 8, 53248);
                    ...
                    tma_3d_gmem2smem(do_panel_addr + stage * 4096, do_tma, half_1 * 32, head_1, trow_1, tiles_full_addr + (stage) * 8);
                    stage += 1;
    ...
    if (warp == 5) {
        { // mma_main
            int half_2 = blockIdx.x % 4;
            int head_2 = blockIdx.x / 4 % num_heads;
            int seq_2 = blockIdx.x / (4 * num_heads);
            int c0_2 = seq_chunk_start[seq_2];
            int nc_2 = seq_chunk_start[seq_2 + 1] - c0_2;
            int row0_2 = c0_2 * 64;
            ...
            if (elect_sync()) {
                ...
                for (int rc_2 = 0; rc_2 < nc_2; rc_2++) {
                    ...
                    int _mma_b_lo_0 = (((dhs_mn_addr) >> 4) & 0x3FFF) | 0x2000000;
                    asm volatile(
                    "{\n\t"
                    ".reg .pred p0, p1;\n\t"
                    ...
                    int _mma_b_lo_1 = ((((do_mn_addr) >> 4) & 0x3FFF) | 0x1000000) + (stage_1) * 256;
                    asm volatile(
                    "{\n\t"
                    ".reg .pred p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
                    ...
                    int _mma_b_lo_2 = (((bdv_mn_addr) >> 4) & 0x3FFF) | 0x1000000;
                    asm volatile(
                    "{\n\t"
                    ".reg .pred p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
                    ...
                    "tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
                    "}\n"
                    :: "r"(_mma_a_lo_2), "r"(_mma_b_lo_2), "r"(tmem_acc_dh), "r"(1), "r"(((uint32_t)(134841488) | ((uint32_t)(1) << 13) | ((uint32_t)(0) << 14))));
