...
kernel_cake_kda_chunk_train_44978dc5595f5354f436(float* __restrict__ dq_intra, float* __restrict__ dk_intra, __nv_bfloat16* __restrict__ q_norm, __nv_bfloat16* __restrict__ k_norm, float* __restrict__ q_rstd, float* __restrict__ k_rstd, int* __restrict__ chunk_bos, int* __restrict__ chunk_len, __nv_bfloat16* __restrict__ dq_out, __nv_bfloat16* __restrict__ dk_out, int num_qk_heads, int num_v_heads, int group)
{
    ...
    for (int it = 0; it < 8; it++) {
        int t = it * 8 + warp_0;
        long long row = row0 + (long long)t;
        ...
        long long qindex = (erow * (long long)num_qk_heads + (long long)head) * 128 + (long long)d0;
        float yq[4];
        float yk[4];
        yq[0] = 0.0f;
        yq[1] = 0.0f;
        ...
        long long rstd_index = erow * (long long)num_qk_heads + (long long)head;
        float rq = 0.0f;
        float rk = 0.0f;
        if (t < clen) {
            rq = q_rstd[rstd_index];
            rk = k_rstd[rstd_index];
        }
        float oq[4];
        float ok[4];
        #pragma unroll
