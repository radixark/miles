...
def test_geo3k_recipe_enables_glm5_next_vision_path(monkeypatch):
    ...
    assert "--offload-rollout-level kv_cache" in train_args
    assert (
        "--custom-model-provider-path miles_plugins.models.glm5_next.vision.glm5_next_vlm_model_provider" in train_args
    )


def test_text_recipe_does_not_load_visual_tower(monkeypatch):
    ...
    assert "--custom-model-provider-path" not in train_args
