...
// Torch host-shim launcher for Cake kernel 'kernel_cake_dsa_train_06715348ab7d777400f6' (host surface: cake_host_shim.h).
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_dsa_train_06715348ab7d777400f6(__nv_bfloat16* __restrict__ q, __nv_bfloat16* __restrict__ kv, float* __restrict__ sink, int* __restrict__ indices, __nv_bfloat16* __restrict__ out, float* __restrict__ lse, int heads, int topk, float scale_log2);


namespace cake_host_shim_86cf26fcbe488eda {
...
void Run(TensorView arg_q, TensorView arg_kv, TensorView arg_sink, TensorView arg_indices, TensorView arg_out, TensorView arg_lse, int64_t arg_heads, int64_t arg_topk, double arg_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_dsa_train_06715348ab7d777400f6),
  ...
  CAKE_HOST_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_dsa_train_06715348ab7d777400f6 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_dsa_train_06715348ab7d777400f6), kargs);
  CAKE_HOST_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_dsa_train_06715348ab7d777400f6 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_86cf26fcbe488eda

CAKE_HOST_EXPORT(run, cake_host_shim_86cf26fcbe488eda::Run);
