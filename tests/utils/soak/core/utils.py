...
from tests.utils.soak.core.events import SoakLaunchFinishedEvent
...
async def note_launch_outcome(
    *, event_log: EventLog, request_id: str | None, launching: Coroutine[Any, Any, None]
) -> Literal["finished", "replaced"]:
    try:
        await launching
    except RunExitedError as error:
        if error.exit_code != REPLACED_LAUNCH_EXIT_CODE:
            event_log.append(SoakLaunchFinishedEvent(request_id=request_id, outcome="failed", error=repr(error)))
            raise
        event_log.append(SoakLaunchFinishedEvent(request_id=request_id, outcome="replaced"))
        return "replaced"
    except BaseException as error:
        event_log.append(SoakLaunchFinishedEvent(request_id=request_id, outcome="failed", error=repr(error)))
        raise
    event_log.append(SoakLaunchFinishedEvent(request_id=request_id, outcome="finished"))
    return "finished"
