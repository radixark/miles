...
@pytest.mark.parametrize(
    ("save_interval", "expected_saves"),
    [
        (3, ["model_save:1:True", "rollout_save:1"]),
        (None, []),
    ],
)
async def test_final_rollout_saves_only_when_a_save_interval_is_configured(monkeypatch, save_interval, expected_saves):
    events = []
    manager = _RecordingRolloutManager(events)
    actor = _RecordingActor(events)
    coordinator = _RecordingCoordinator(events=events, manager=manager)
    ...
    await train_async.train(_args(num_rollout=2, save_interval=save_interval))

    assert "train:0" in events
    assert "train:1" in events
    assert [event for event in events if event.startswith(("rollout_save:", "model_save:"))] == expected_saves
