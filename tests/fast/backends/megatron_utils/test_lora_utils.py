...
class TestHfPeftExport:
    ...
    def test_rank0_write_failure_recovers_before_outer_barrier(self, tmp_path, monkeypatch):
        import miles.backends.megatron_utils.lora_utils as lora_utils
        ...
        def run_rank(rank, *, fail_during_rank0_write):
            bridge = MagicMock()
            self._install_bridge_stub(monkeypatch, bridge)
            monkeypatch.setattr("miles.utils.megatron_bridge_utils.patch_megatron_model", lambda _: nullcontext())
            ...
            def all_gather_object(output, value):
                ...
                if consensus_count == 0:
                    ...
                    events.append("setup-consensus")
                    output[:] = [None, None]
                else:
                    assert (value == "OSError('disk full')") is (rank == 0)
                    events.append("save-consensus")
                    output[:] = ["OSError('disk full')", None]
                consensus_count += 1

            monkeypatch.setattr(lora_utils.dist, "barrier", barrier)
            monkeypatch.setattr(lora_utils.dist, "all_gather_object", all_gather_object)

            def save_hf_adapter(*_args, **_kwargs):
                lora_utils.dist.barrier()  # Bridge initial barrier.
                if fail_during_rank0_write:
                    events.append("rank-0-write-failed")
                    raise OSError("disk full")
                lora_utils.dist.barrier()  # Bridge final barrier.

            bridge.save_hf_adapter.side_effect = save_hf_adapter
            model = MagicMock()
            model.named_parameters.return_value = []
            optimizer = MagicMock()
            optimizer.state_dict.side_effect = lambda: events.append("training-state") or {}

            lora_utils.save_lora_checkpoint(
                [model],
                args,
                str(tmp_path / f"rank{rank}"),
                optimizer=optimizer,
            )
            return events, collectives

        rank0_events, rank0_collectives = run_rank(0, fail_during_rank0_write=True)
        peer_events, peer_collectives = run_rank(1, fail_during_rank0_write=False)
