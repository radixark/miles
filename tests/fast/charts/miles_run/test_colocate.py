...
def pool_pod(objects: list[dict[str, Any]], name: str) -> dict[str, Any]:
    pool_id = named_object(objects, "LeaderWorkerSet", name)
    return pool_id["spec"]["leaderWorkerTemplate"]["workerTemplate"]["spec"]
...
class TestPairingController:
    ...
    def test_is_told_the_object_names_and_cell_counts_the_chart_derived(self):
        """Values name the pool_id entries, so only the chart knows the object names and their group shapes."""
