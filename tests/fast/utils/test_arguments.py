...
class TestTitoFixedTemplateConfiguration:
    ...
    @pytest.mark.parametrize("family", ["qwen35", "qwen36"])
    def test_qwen35_and_qwen36_resolve_shared_template(self, family):
        args = self._parse(["--use-session-server", "--tito-model", family])
        miles_validate_args(args)
        assert args.chat_template_path.endswith("/qwen3.5_and_3.6_fixed.jinja")
