...
if [[ "$TASK" == "dapo-math" ]]; then SEQ="${SEQ:-4096}"; elif [[ "$TASK" == "gsm8k" ]]; then SEQ="${SEQ:-256}"; else SEQ="${SEQ:-8192}"; fi  # target seq window; gsm8k short-answer -> 256 (not a native flag)
# ----------------------------------------------------------------------------
...
if [[ "$TASK" == "dapo-math" ]]; then RESP_LEN="${RESP_LEN:-3584}"; elif [[ "$TASK" == "gsm8k" ]]; then RESP_LEN="${RESP_LEN:-256}"; else RESP_LEN="${RESP_LEN:-7168}"; fi  # --rollout-max-response-len (gsm8k -> 256 within SEQ)
