...
from tests.utils.soak.ft.checkers.resize import LANDING_LAG_ROLLOUTS, assert_counts_follow_schedule
from tests.utils.soak.ft.types import ACTOR_CELL_TYPE, ResizeStep, compute_sizes
...
SCHEDULE: tuple[ResizeStep, ...] = compute_scaling_schedule()
...
def _assert_trainer_cells_scaled(events: list[SoakEvent], dump_dir: str) -> None:
    ...
    assert_counts_follow_schedule(
        ...
        subject="the number of trainer cells that trained a step",
    )
...
def _read_num_trained_cells_of_rollout(events_dir: Path) -> dict[int, int]:
    ...
    for rollout_id, event in sorted(final_of_rollout.items()):
        removed = _compute_removed_cell_indices(rollout_id)
        assert all(
            ...
            f"the final attempt {event.attempt} of rollout {rollout_id} ended with {event.cell_outcomes}, and only "
            f"the cell(s) {sorted(removed)} a shrink removes around then may fail in it, so a cell that a resize "
            f"added or removed took the step down with it instead of joining or leaving cleanly"
        )
...
def _compute_removed_cell_indices(rollout_id: int) -> set[int]:
    sizes = compute_sizes(initial_replicas=_MODE.num_cells, schedule=SCHEDULE)
    return {
        cell_index
        for step, size_before in zip(SCHEDULE, sizes[:-1], strict=True)
...
def _assert_reconfigured_once_per_resize(events_dir: Path) -> None:
    ...
    for step, info in zip(SCHEDULE, actual, strict=True):
        assert step.at_rollout <= info.rollout_id <= step.at_rollout + LANDING_LAG_ROLLOUTS, (
            f"the resize fired during rollout {step.at_rollout} reconfigured the quorum at rollout "
