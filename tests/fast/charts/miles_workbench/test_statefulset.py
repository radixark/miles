...
class TestWorkbenchStatefulSet:
    ...
    def test_the_statefulset_name_leaves_room_for_the_names_derived_from_it(self):
        """Pod hostnames and the controller-revision-hash label are 63-char labels built from this name."""
        long_release = "a" * 53
        result = subprocess.run(
            ["helm", "template", long_release, str(CHART_DIR), "-n", NAMESPACE],
            capture_output=True,
            text=True,
        )
        objects = [document for document in yaml.safe_load_all(result.stdout) if document is not None]
        name = single_object_of_kind(objects, "StatefulSet")["metadata"]["name"]
