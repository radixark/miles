...
The dispatched workflow and its reusable workflows come from the reviewed default-branch commit. A trusted resolver reads the separately checked-out PR snapshot without importing it and maps the requested registration to a fixed CPU/CUDA suite, runner, image, and timeout. It parses the snapshot's `KNOWN_LABELS` literal dictionary as AST data before validating registrations, so a PR can add a domain label and tests using it atomically without default-branch schema skew; dynamic, malformed, duplicate, or symlinked label registries fail closed. The execution job then checks out that same exact SHA and uses the requested path as its command entrypoint: bounded `pytest` for CPU or bounded `python3` for CUDA. It does not depend on workflow or runner code from the PR snapshot.
