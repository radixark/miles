...
| `MOONCAKE_SOS_COMMIT`                                                                                  | kvcache-ai/Mooncake commit whose `structured_object_store.py` is overlaid onto the base image's mooncake wheel (the rollout-data-transport API is newer than any released wheel); drop the overlay once mooncake ships it in a release                                                                                                              |
