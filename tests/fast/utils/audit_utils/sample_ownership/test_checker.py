...
async def test_completed_step_window_controls_analysis(
    ...
    """The trainer cutoff gates analysis and defines the mature sample boundary."""
    ...
    class Store:
        def read_current(self) -> SimpleNamespace:
            return SimpleNamespace(marker=SimpleNamespace(mature_before=cutoff))
    ...
    monkeypatch.setattr(
        checker_module, "compute_trainer_configs", lambda args: [SimpleNamespace(role="actor", trainer_id="actor")]
    )
    monkeypatch.setattr(checker_module, "get_backend_capability", lambda args: None)
    monkeypatch.setattr(checker_module, "create_trainer_controller_handle", lambda *args, **kwargs: Controller())
    ...
    assert calls == ([7, "analysis"] if has_mature_samples else [7])
