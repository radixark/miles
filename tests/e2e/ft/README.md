...
Assertions:
  ...
  6. Numerical prerequisites, per side: every trainer rank and every updated engine incarnation
...
- **Why the numerical-prerequisite witness**: bitwise equality proves nothing if a replacement engine came up without the deterministic recipe the baseline had, so each engine incarnation that took weights must report it. The analyzer rule `engine_env_report_coverage` requires the report itself before every training step; the recipe it must contain is known only to the scenario, so `assert_deterministic_environment` compares it after the run.
...
- **Hook topology**: `--mix` adds `--update-weights-timeout 600`; it sets trainer faults at both `trainer_weight_update_before_all_gather` and `trainer_weight_update_before_send`, and, when rollout ft is on, receiver faults triggered at `trainer_weight_update_before_send`.
