...
def _capture_backend(monkeypatch) -> list[str]:
    chosen: list[str] = []

    class _Recorded:
        def execute_train(self, request) -> None: ...

    def _resolve(cluster_backend: str, *, train_args: str):
        chosen.append(cluster_backend)
        return _Recorded()

    monkeypatch.setattr(command_utils, "_resolve_backend", _resolve)
    return chosen
...
class TestATrainingE2eTestRunsOnWhicheverBackendItsEnvironmentNames:
    ...
    def test_an_unset_environment_still_means_ray(self, monkeypatch):
        ...
        assert chosen == [ClusterBackend.RAY.value]
    ...
    def test_a_command_run_before_the_launch_reaches_the_same_backend(self, monkeypatch):
        ...
        assert base_backend.active_backend().__class__.__name__ == "KubernetesCommandBackend"
