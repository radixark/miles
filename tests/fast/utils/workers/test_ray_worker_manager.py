...
class TestComputeCellMeta:
    ...
    def test_a_spec_without_meta_reports_none(self):
        ...
        assert compute_cell_meta(_make_spec("engine"), cell_id="engine-0", cell_ordinal=0) == {}
    ...
    def test_meta_that_names_no_gpu_span_is_passed_through(self):
        ...
        assert compute_cell_meta(spec, cell_id="engine-a", cell_ordinal=3) == {"role": "engine"}
    ...
    def test_single_gpu_cells_carry_contiguous_gpu_offsets(self):
        ...
        offsets = [
            compute_cell_meta(spec, cell_id=f"engine-{index}", cell_ordinal=index)["gpu_offset"] for index in range(8)
        ]
    ...
    def test_multi_node_cells_advance_by_a_whole_engine(self):
        ...
        offsets = [
            compute_cell_meta(spec, cell_id=f"engine-{index}", cell_ordinal=index)["gpu_offset"] for index in range(2)
        ]
    ...
    def test_the_specs_own_offset_shifts_every_cell(self):
        ...
        offsets = [
            compute_cell_meta(spec, cell_id=f"engine-{index}", cell_ordinal=index)["gpu_offset"] for index in range(16)
        ]
