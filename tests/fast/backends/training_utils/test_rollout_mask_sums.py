...
class TestAggregateTrainLossesConstantDivisor:
    ...
    def _mb(self, denominator, values):
        return {"keys": list(values.keys()), "values": torch.tensor([denominator, *values.values()])}

    def test_constant_divisor_mode(self):
        # producer left the 0 placeholder -> divide by step_global_batch_size, no cp factor
        losses = [self._mb(0, {"loss": 2.0}), self._mb(0, {"loss": 6.0})]
        out = log_utils.aggregate_train_losses(losses, step_global_batch_size=4)
    ...
    def test_legacy_values_win_when_producer_not_in_placeholder_mode(self):
        """A non-zero values[0] (legacy producer) keeps the legacy reduction even
        if the caller passes a step size."""
        losses = [self._mb(4, {"loss": 8.0})]
        out = log_utils.aggregate_train_losses(losses, step_global_batch_size=999)
        assert math.isclose(out["loss"], 2.0)
