...
- Pre-fault rollouts are compared on the target's own live samples, which only works while both sides push bitwise-identical weights to the engines: any bf16 ulp in a weight flips temperature-0.8 samples and the fault rollout then trains different data (observed 6% `train/grad_norm` gap). Two things keep the weights bitwise up to the fault, on **both** sides: `--debug-deterministic-collective` (same fixed fold for the normal-DP 4-rank reduce and the indep_dp CP-then-cross-cell reduce, as in `scenario_deterministic`) and `--clip-grad 10.0` (clipping inactive; the dense model's grad norm is ~1.3, and `train/grad_norm` differs by a few fp32 ulps across shardings, which an active clip would multiply into every update). The other FT modes have grad norms below 1.0, so clipping is inactive there without the override.
