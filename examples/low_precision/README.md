...
Following the above command will launch FP8 training. According to miles's design, if the model under `--hf-checkpoint` is FP8, it will automatically use FP8 quantization in weight updates.
...
3. Update weight: In RL weight updates, the training engine will attempt to save model weights. The saved results will be dequantized from FP8 to bf16, but since the config under `--hf-checkpoint` is FP8, miles will quantize this bf16.
