...
@pytest.mark.parametrize(
    ...
    [
        ...
        ((10, 0), 2000, None, None, False),  # not a multiple of 128
        ((10, 0), 512, [0, 256, 512], None, True),  # equal-length packed
        ((10, 0), 384, [0, 128, 384], None, False),  # unequal packed lengths
...
@pytest.mark.parametrize(
    "seq_len, cu_seqlens",
    [(256, None), (512, None), (512, [0, 256, 512])],
    ids=["t256", "t512", "packed_2x256"],
)
def test_deterministic_backend_matches_fla(seq_len, cu_seqlens):
    """Same forward bits; gradients within the bf16 tolerance of FLA's own backward."""
...
def test_deterministic_backend_backward_is_bit_deterministic():
    _require_gpu_backends()
    module = load_backend_module()
    inputs = _inputs(512, seed=461)
    _, first = _run(module, "deterministic", inputs, None)
    _, second = _run(module, "deterministic", inputs, None)
    for name in _GRADS:
        assert torch.equal(first[name], second[name]), f"{name} differs between identical backward calls"


def test_deterministic_backend_falls_back_outside_domain():
    """A call the deterministic backward does not cover (T not a multiple of 128) still trains, through FLA."""
    _require_gpu_backends()
    module = load_backend_module()
