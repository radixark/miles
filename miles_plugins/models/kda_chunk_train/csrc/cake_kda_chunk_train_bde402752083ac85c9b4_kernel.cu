...
kernel_cake_kda_chunk_train_bde402752083ac85c9b4(CakeTensorMap const* kg_tma, CakeTensorMap const* qg_tma, CakeTensorMap const* w_tma, CakeTensorMap const* do_tma, __nv_bfloat16* __restrict__ dv1, float* __restrict__ gk, __nv_bfloat16* __restrict__ dh_out, __nv_bfloat16* __restrict__ dv2, int num_heads, int* __restrict__ seq_chunk_start, float scale)
{
    ...
    if (warp == 5) {
        { // mma_main
            int half_2 = blockIdx.x % 2;
            int head_2 = blockIdx.x / 2 % num_heads;
            int seq_2 = blockIdx.x / (2 * num_heads);
