...
from miles.utils.types import LEGACY_WEIGHT_VERSIONS_KEY, Sample, WeightVersionsPerCall, prefill_lag
...
class DumpReader:
    ...
    def _summary_row(self, sample: Sample, row: TrainRow | None, *, rollout_id: int, sample_occurrence: int) -> dict:
        ...
        prefill_lags = [x for call in sample.weight_versions if (x := prefill_lag(call)) is not None]
        entry = dict(
