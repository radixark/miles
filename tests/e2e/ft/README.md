...
- CI runs it via thin per-mode entry files `test_trainer_ft_<scenario>_<mode>.py`, each registered with `register_cuda_ci(est_time=..., suite="stage-c-8-gpu-h200", labels=["ft-fast"])` (comparison scenarios) or `labels=["ft-slow"]` (soak scenarios).
...
- Gated on the `run-ci-ft-fast` / `run-ci-ft-slow` PR labels (FT is expensive — not run on every PR). `ft-fast` covers the comparison scenarios (no_failure / deterministic / with_failure, minutes each); `ft-slow` covers the soak scenarios (random-crash survival, realistic-gsm8k convergence — tens of minutes to hours). With a label set, the matching entries run on `stage-c-8-gpu-h200`.
