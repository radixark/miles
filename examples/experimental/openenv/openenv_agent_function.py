...
async def multi_turn(
    ...
    *,
    ...
    async def body(env: Any) -> tuple[float | None, str | None, int, str, list[float], list[float], float, float]:
        ...
        # No canonical verdict -> reward None, with the reason for run_for_training
        # to classify:
...
async def run_for_training(
    ...
    see miles.rollout.agentic.agent_function): a sandbox that could not be
    created, a server that does not carry the canonical scoring contract.
    Everything else -- the wall-clock cap, an env that broke mid-episode, a
    scoring step that errored -- is the policy's and scores 0; a discarded
    outcome the policy can trigger would teach it to trigger it.
    """
