...
def test_checkpointed_cross_entropy_matches_dense_forward_and_backward() -> None:
    ...
    hidden = torch.randn(7, 2, 5, requires_grad=True)
    labels = torch.tensor([[1, 2, 3, 4, -100, 6, 7], [7, 6, 5, 4, 3, 2, 1]])
    actual = checkpointed_vocab_parallel_cross_entropy(
        hidden, labels, output_layer=layer, output_weight=None, chunk_size=3, sequence_parallel_input=False
    )
    actual.sum().backward()
    ...
    expected = F.cross_entropy(dense_logits.flatten(0, 1), safe_labels.flatten(), reduction="none").view_as(
        safe_labels
    )
...
def test_sft_output_processor_selects_shifted_response_positions(monkeypatch) -> None:
    ...
    hidden = torch.randn(6, 1, 4, requires_grad=True)
    tokens = torch.tensor([10, 11, 12, 13, 14, 15])
    args = SimpleNamespace(qkv_format="thd", allgather_cp=False, rollout_temperature=1.0, true_on_policy_mode=False)
    context = SFTCheckpointedOutputContext(
        args=args,
        batch={"unconcat_tokens": [tokens], "total_lengths": [6], "response_lengths": [3], "max_seq_lens": None},
        chunk_size=2,
    )
    ...
    expected = F.log_softmax(F.linear(hidden[2:5, 0], layer.weight), dim=-1).gather(-1, tokens[3:6, None])
