...
from tests.e2e.ft.conftest_ft.fault_injection import EventLog, cell_is_alive
...
def test_a_fresh_log_counts_every_healthy_cell_as_alive() -> None:
    ...
    log.observe(cells)
    assert _names(fi.compute_genuinely_alive(log.events, cells)) == {"c0"}
...
def test_injected_cell_is_excluded_while_its_crash_is_still_undetected() -> None:
    ...
    assert _names(fi.compute_genuinely_alive(log.events, cells)) == {"c1"}
...
def test_injected_cell_counts_again_only_after_a_full_down_then_up_cycle() -> None:
    ...
    assert _names(fi.compute_genuinely_alive(log.events, healthy)) == {"c1"}
    log.observe(down)  # detected down
    assert _names(fi.compute_genuinely_alive(log.events, down)) == {"c1"}
    log.observe(healthy)  # healed
    assert _names(fi.compute_genuinely_alive(log.events, healthy)) == {"c0", "c1"}
...
def test_vanished_cell_counts_as_the_down_half_of_the_cycle() -> None:
    ...
    log = EventLog()
    log.note_injected("c0")
    log.observe([_cell("c1", healthy=True)])  # c0 absent == down
    healthy = [_cell("c0", healthy=True), _cell("c1", healthy=True)]
    log.observe(healthy)
    assert _names(fi.compute_genuinely_alive(log.events, healthy)) == {"c0", "c1"}
...
def test_allows_overlapping_crashes_while_one_cell_stays_alive() -> None:
    ...
    log.note_injected("c0")
    log.observe(cells)
    assert _names(fi.compute_genuinely_alive(log.events, cells)) == {
        "c1",
        "c2",
    ...
    log.observe(cells)
    assert _names(fi.compute_genuinely_alive(log.events, cells)) == {"c2"}  # now only 1 -> loop would skip
...
def _log_of(states: list[fi.ObservedCellState], *, inject_before: dict[int, int] | None = None) -> fi.EventLog:
    log = fi.EventLog()
    for index, state in enumerate(states):
        for _ in range((inject_before or {}).get(index, 0)):
            log.note_injected("rollout-engine-0")
        log.observe([_staged("rollout-engine-0", state)])
    return log
...
def test_observed_states_record_only_transitions() -> None:
    ...
    log = _log_of([_SERVING, _SERVING, _SUSPENDED, _SUSPENDED, _SERVING])

    assert fi.compute_states_of_cell_name(log.events) == {"rollout-engine-0": [_SERVING, _SUSPENDED, _SERVING]}
...
def test_a_colocated_cell_recovers_through_a_gated_relaunch() -> None:
    ...
    log = _log_of([_SERVING, _SUSPENDED, _PENDING, _RUNNING_NOT_SERVING, _SERVING], inject_before={1: 1})
...
def test_a_missed_suspended_sample_still_counts_as_a_recovery() -> None:
    ...
    log = _log_of([_SERVING, _PENDING, _SERVING], inject_before={1: 1})
...
def test_a_replacement_that_never_reaches_the_router_is_not_a_recovery() -> None:
    ...
    log = _log_of([_SERVING, _PENDING, _RUNNING_NOT_SERVING], inject_before={1: 1})

    assert fi.compute_num_completed_recoveries(log.events, cell_type="rollout") == 0
    assert fi.compute_cells_with_unfinished_recovery(log.events, cell_type="rollout") == {"rollout-engine-0": 1}
...
def test_a_cell_that_was_never_injected_witnesses_no_recovery() -> None:
    ...
    log = _log_of([_PENDING, _SERVING])

    assert fi.compute_num_injections(log.events, cell_type="rollout") == 0
    assert fi.compute_num_completed_recoveries(log.events, cell_type="rollout") == 0
...
def test_skipping_the_relaunch_phase_is_not_a_recovery() -> None:
    ...
    log = _log_of([_SERVING, _RUNNING_NOT_SERVING, _SERVING], inject_before={1: 1})
...
def test_each_accepted_injection_needs_its_own_completed_recovery() -> None:
    ...
    log = _log_of([_SERVING, _PENDING, _SERVING, _SERVING], inject_before={1: 1, 3: 1})

    assert fi.compute_num_injections(log.events, cell_type="rollout") == 2
    assert fi.compute_num_completed_recoveries(log.events, cell_type="rollout") == 1
    assert fi.compute_cells_with_unfinished_recovery(log.events, cell_type="rollout") == {"rollout-engine-0": 1}
...
def test_recoveries_of_another_cell_kind_do_not_count() -> None:
    ...
    log = fi.EventLog()
    log.observe([_staged("actor-0", _SERVING, cell_type="actor")])
    log.note_injected("actor-0")
    for state in [_PENDING, _SERVING]:
        log.observe([_staged("actor-0", state, cell_type="actor")])

    assert fi.compute_num_completed_recoveries(log.events, cell_type="rollout") == 0
    assert fi.compute_num_completed_recoveries(log.events, cell_type="actor") == 1
...
def test_stop_and_join_takes_one_last_snapshot_before_the_log_is_read() -> None:
    ...
    assert fi.compute_states_of_cell_name(handle.event_log.events) == {"rollout-engine-0": [_SERVING]}
...
class TestRecoveryPairing:
    def test_another_cells_relaunch_cannot_complete_the_injected_cells_recovery(self) -> None:
        ...
        log = fi.EventLog()
        log.observe([_staged("rollout-engine-0", _SERVING), _staged("rollout-engine-1", _SERVING)])
        log.note_injected("rollout-engine-0")
        for sibling_state in [_PENDING, _SERVING]:
            log.observe([_staged("rollout-engine-0", _SERVING), _staged("rollout-engine-1", sibling_state)])

        assert fi.compute_num_injections(log.events, cell_type="rollout") == 1
        assert fi.compute_num_completed_recoveries(log.events, cell_type="rollout") == 0
        assert fi.compute_cells_with_unfinished_recovery(log.events, cell_type="rollout") == {"rollout-engine-0": 1}
    ...
    def test_relaunch_observed_before_injection_does_not_count_as_recovery(self) -> None:
        ...
        log = _log_of([_SERVING, _PENDING, _SERVING], inject_before={2: 1})
...
class TestFaultInjectionLoopErrorHandling:
    def test_list_cells_failure_is_retried_without_recording_recovery(self) -> None:
        ...
        def fake_get(url: str, timeout: float) -> MagicMock:
            polls["n"] += 1
            if polls["n"] in {2, 3}:
                debt_around_failure.append(fi.compute_cells_with_unfinished_recovery(log.events, cell_type="rollout"))
            if polls["n"] == 2:
        ...
        assert fi.compute_states_of_cell_name(log.events) == {
            "rollout-engine-0": [_SERVING],
            "rollout-engine-1": [_SERVING],
        }
        assert fi.compute_num_injections(log.events, cell_type="rollout") == 1
        assert fi.compute_num_completed_recoveries(log.events, cell_type="rollout") == 0
        assert fi.compute_cells_with_unfinished_recovery(log.events, cell_type="rollout") == expected_debt
