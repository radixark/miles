...
Distinct from image selection, the **`run-ci-image` label** selects the test scope — every enabled tag except `long`, `ft-short`, `ft-long`, and `verifiers` — which validates an image bump without selecting those domains implicitly.
