...
def _mock_megatron_environment():
    ...
    try:
        ...
        # Surgical restore, NOT sys.modules.clear(): evicting every module
        # imported during this file's window forces later tests' imports to
        # re-execute third-party module bodies whose registrations are
        # one-shot (torch's mega-cache artifact factory asserts on the
        # duplicate), and leaves stale submodule attributes on retained
        # parent packages. Drop only the namespaces the stubs poisoned;
        # restore every original entry over the stubs.
