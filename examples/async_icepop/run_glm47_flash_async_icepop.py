...
def execute(args: ScriptArgs):
    ...
    if args.async_mode:
        # Async: TP=4, EP=4 → 4 GPUs for actor, 4 GPUs for rollout on single node
        perf_args = (
        ...
        # Sync: TP=4, EP=8 → 8 GPUs colocated
        perf_args = (
            "--tensor-model-parallel-size 4 "
            "--sequence-parallel "
            "--pipeline-model-parallel-size 1 "
    ...
    if args.async_mode:
        # Async: 4 GPUs actor + 4 GPUs rollout, no colocate
        misc_args = (
