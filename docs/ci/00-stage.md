...
**Accelerator isolation.** A GPU test file runs `train.py` under Ray, so the SGLang schedulers and Ray workers holding device memory are grandchildren of the test process; a file that dies before its own teardown leaves them running, and the next file starts on a device that is already occupied. The CUDA suite runner therefore reaps between files: before each attempt it runs `ray stop --force`, kills any surviving `sglang::` and `ray::` processes, and waits for the driver to release the memory asynchronously. The hosted CPU suites do not reap — `run_unittest_files` is itself under test there, and a process-wide kill would take down the pytest process running it.
