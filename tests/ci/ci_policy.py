...
def resolve_policy(cadence: str, raw_labels: set[str]) -> RunPolicy:
    ...
    - Nightly cadence excludes `long`, `ft-long`, and `verifiers`.
    - `run-ci-image` excludes `long`, `ft-short`, `ft-long`, and `verifiers`.
    ...
    if "run-ci-all" in raw_labels or cadence in {WEEKLY_CADENCE, RELEASE_CADENCE}:
        ...
        scope = set(KNOWN_LABELS) - {"long", "ft-long", "verifiers"}
    elif "run-ci-image" in raw_labels:
        scope = set(KNOWN_LABELS) - {"long", "ft-short", "ft-long", "verifiers"}
    else:
        scope = set()
