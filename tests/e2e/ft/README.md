...
Phase A (both baseline and target, normal DP):
...
- Phase A uses normal DP on both sides so the checkpoint seed is identical before the compared phase begins. Phase B keeps the intended normal-DP baseline versus indep-DP target comparison; only the target executes crash→retry→heal.
