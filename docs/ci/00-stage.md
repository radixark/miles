...
A **nightly** run selects every enabled tag except `ft-long`, with fast-fail off. It is triggered either by the nightly `schedule` cron on `main` or by a PR carrying the `nightly` label, and turns off fast-fail like the `bypass-fastfail` label.

`run-ci-all` and a manual `workflow_dispatch` select the full tag set, so every enabled tag is included. If scope labels overlap, the precedence is `run-ci-all` > nightly > `run-ci-image`. The whole trigger→scope mapping lives in `run_suite.py` `resolve_scope`, which outputs one include-label set (see docs/ci/01-label.md for the subtraction semantics); each workflow stage passes only the raw PR labels and `--event-name`.
