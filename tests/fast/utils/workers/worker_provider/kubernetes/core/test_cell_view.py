...
def make_run(*, ranks_per_pod: int = 1) -> KubernetesRunInfo:
    return KubernetesRunInfo(
        namespace="rl",
        label_selector="app.kubernetes.io/instance=r",
        specs={
            "engine": make_pool_spec(
                "engine", ports={"rpc": 8000}, worker_class=ENGINE_CLASS, ranks_per_pod=ranks_per_pod
            )
        },
...
def build_worker_infos(pods: list[ParsedPod], *, ranks_per_pod: int = 1):
    return compute_worker_infos(
        ...
        worker_classes={"engine": ENGINE_CLASS},
    )
