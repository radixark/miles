...
class TestComputeDynamicGlobalBatchSize:
    def test_rounds_down_to_multiple_of_dp_size(self):
        ...
        gbs = _compute_dynamic_global_batch_size(
            args=args, train_parallel_config=make_train_parallel_config(dp_size=4), num_samples=13
        )
        assert gbs == 12
    ...
    def test_returns_num_samples_when_already_aligned(self):
        args = make_args(global_batch_size=64)
        gbs = _compute_dynamic_global_batch_size(
            args=args, train_parallel_config=make_train_parallel_config(dp_size=4), num_samples=16
        )
        assert gbs == 16
    ...
    def test_falls_back_to_dp_size_when_below_dp_size(self):
        ...
        args = make_args(global_batch_size=64)
        gbs = _compute_dynamic_global_batch_size(
            args=args, train_parallel_config=make_train_parallel_config(dp_size=4), num_samples=2
        )
        assert gbs == 4
...
class TestPostprocessRolloutData:
    ...
    def test_raises_when_trim_would_produce_zero_samples(self):
        ...
        with pytest.raises(ValueError, match="Not enough samples"):
            postprocess_rollout_data(
                args=args, data=[make_sample()] * 5, train_parallel_config=make_train_parallel_config(dp_size=1)
            )
    ...
    def test_flattens_nested_list_of_lists(self):
        ...
        out, _meta = postprocess_rollout_data(
            args=args, data=nested, train_parallel_config=make_train_parallel_config(dp_size=1)
        )
        assert len(out) == 3
