...
* You completed [Customization](/miles/docs/user-guide/customization) — this example uses a
...
2. **Tokenization alignment.** The model must see and the trainer must score the
   *exact same tokens*. Pre-tokenizing vs. re-tokenizing at training time can drift —
   that's where the [chat template verifier](/miles/docs/user-guide/agentic-chat-template)
   matters.
