...
class ReloadableProcessGroup(torch.distributed.ProcessGroup):
    ...
    def _adopt_inner_backends(self) -> None:
        """Register the inner group's backends on this wrapper; re-run after every reload.

        torch 2.13 dispatches collectives through the C++ backend map of the group object
        itself, which is empty on the bare wrapper, so they fail with "No backend type
    ...
    # No python overrides for collectives: each would become a pybind trampoline that
    # wraps the Work in PyWorkHolder, which segfaults in wait() on torch 2.13. The
    # registered inner backends serve collectives through the C++ dispatch instead.
