...
1. Create a GitHub App, install it only on `radixark/miles`, and grant `Issues: write` and `Pull requests: write`; do not grant `Actions: write` or `Contents: write`. A label lands on an issue that is a pull request, and GitHub gates issues-API calls whose target issue is a PR on the pull-requests scope, so `Pull requests: read` makes the label `POST` fail with HTTP 403. When a permission is added to an already installed App, an organization administrator must also accept it on the installation, or minting the token fails. The App token is minted only for label commands; the actions-capability and feedback jobs never receive it.
