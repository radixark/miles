...
SCORING_DTYPES = (torch.bfloat16, torch.float32)


def _tolerances(dtype: torch.dtype) -> tuple[float, float]:
    return (2e-5, 2e-5) if dtype == torch.float32 else (1e-1, 1e-2)


@pytest.mark.parametrize("dtype", SCORING_DTYPES)
def test_batch_invariant_logsoftmax_opcheck(dtype):
    logits = torch.randn(
        3,
        1025,
        device="cuda",
...
def test_batch_invariant_logsoftmax_contract_fails_closed():
    ...
    assert batch_invariant_module.batch_invariant_log_softmax(logits.to(torch.bfloat16)).dtype == torch.bfloat16
    with pytest.raises(TypeError, match="requires BF16 or FP32 input"):
        batch_invariant_module.batch_invariant_log_softmax(logits.to(torch.float16))
...
@pytest.mark.parametrize("dtype", SCORING_DTYPES)
def test_batch_invariant_logsoftmax_matches_sglang_and_is_outer_shape_invariant(vocab_size, dtype):
    ...
    torch.testing.assert_close(actual, torch_reference, atol=atol, rtol=rtol)


@pytest.mark.parametrize("dtype", SCORING_DTYPES)
def test_batch_invariant_logsoftmax_backward_uses_exact_forward_output(dtype):
    logits = torch.randn(
        3,
        4097,
        device="cuda",
    ...
    atol, rtol = _tolerances(dtype)
    torch.testing.assert_close(actual_grad, independent_grad, atol=atol, rtol=rtol)
    assert torch.isfinite(actual_grad).all()
    assert torch.count_nonzero(actual_grad) > 0


@pytest.mark.parametrize("dtype", SCORING_DTYPES)
def test_selected_token_loss_backward_matches_torch_reference(dtype):
    ...
    atol, rtol = _tolerances(dtype)
    torch.testing.assert_close(actual_logits.grad, reference_logits.grad, atol=atol, rtol=rtol)


@pytest.mark.parametrize("dtype", SCORING_DTYPES)
def test_scoring_truncates_padded_vocab_and_casts_only_selected_logprobs(dtype):
    ...
    assert torch.count_nonzero(padded_logits.grad[:, 1025:]) == 0
    atol, rtol = _tolerances(dtype)
    torch.testing.assert_close(padded_logits.grad[:, :1025], reference_logits.grad, atol=atol, rtol=rtol)


@pytest.mark.parametrize("dtype", SCORING_DTYPES)
def test_batch_invariant_logsoftmax_preserves_explicit_dtype_under_autocast(dtype):
    logits = torch.randn(3, 1025, device="cuda", dtype=dtype)

    expected = batch_invariant_module.batch_invariant_log_softmax(logits)
...
@pytest.mark.parametrize("dtype", SCORING_DTYPES)
def test_batch_invariant_logsoftmax_fullgraph_compile_forward_and_backward(dtype):
