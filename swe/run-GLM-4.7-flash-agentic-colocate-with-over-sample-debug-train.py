...
task = "swegym"
DEBUG_ROLLOUT_DATA = (
    "/fs/nlp-intern/yangchengyi/trajs/"
    "swegym_GLM-4.7-Flash_agentic_colocate_over_sample_20260531_110011/"
    "rollout_data/0.pt"
)

@dataclass
class ScriptArgs(U.ExecuteTrainConfig):
    mode: Literal["normal", "debug_rollout_only","debug_train_only"] = "debug_train_only"
...
def execute(args: ScriptArgs):
    ...
    # Debug-train target: first make initialization fit on 4 nodes / 32 H100s.
    # PP=4 reduces per-rank parameter and fp32 grad-buffer memory; CP=2 keeps
    # some long-sequence splitting for the 65k-token debug rollout.
    tp, pp, cp = 4, 4, 2
    ...
    debug_args = ""
    if args.mode == "debug_train_only":
        debug_args += (
            "--debug-train-only "
            f"--load-debug-rollout-data {DEBUG_ROLLOUT_DATA} "
        )
    elif args.mode == "debug_rollout_only":
