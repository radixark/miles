...
| `watchHandler` per-event handling failure | One malformed frame must not stop the watch | **Kept** when the envelope has a cursor: log, skip, advance past it. An unreadable envelope relists instead | Tearing down reconnects at the same cursor replays the same frame, while relisting can fail on the same malformed pod. A frame with no readable cursor cannot be skipped safely |
...
| Frame payload type | Only some frames carry a pod | **Kept**: `PodWatchEvent.pod` is validated for `ADDED` / `MODIFIED` / `DELETED` only. LIST rejects an unreadable pod; WATCH preserves a readable envelope and its validation error so the reflector can log, skip and advance | `BOOKMARK` carries a bare `resourceVersion` and `ERROR` carries a `Status`, so validating either as a pod would fail on every frame. LIST must never publish a partial snapshot. WATCH already has a snapshot, and relisting on a malformed payload can fail on the same still-present pod forever; preserving the envelope separates that recoverable per-event failure from an unreadable frame |
