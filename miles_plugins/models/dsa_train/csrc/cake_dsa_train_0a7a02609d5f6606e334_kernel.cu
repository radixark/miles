...
kernel_cake_dsa_train_0a7a02609d5f6606e334(__nv_bfloat16* __restrict__ index_q, __nv_bfloat16* __restrict__ index_k, float* __restrict__ weights, int* __restrict__ cu_ks, int* __restrict__ cu_ke, float* __restrict__ logits, int seq_len, int seq_len_kv)
{
    ...
    long long row0 = ((long long)b * (long long)seq_len + (long long)q0) * 64;
    ...
    if (tid_0 < 64) {
        float w_val = 0.0f;
        if (q0 + tid_0 / 64 < seq_len) {
            w_val = weights[row0 + (long long)tid_0];
        }
        s_w[tid_0] = w_val;
    }
    asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
        :: "r"(s_q_addr + (unsigned int)(tid % 16 * 8 / 64 * 8192 + (tid / 16 * 128 + tid % 16 * 8 % 64 * 2 ^ (tid / 16 * 128 + tid % 16 * 8 % 64 * 2 >> 7 & 7) << 4))), "l"(index_q + ((row0 + (long long)(tid / 16)) * 128 + (long long)(tid % 16 * 8))), "r"((q0 + tid / 16 / 64 < seq_len) ? 16 : 0));
    asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
        :: "r"(s_q_addr + (unsigned int)((256 + tid) % 16 * 8 / 64 * 8192 + ((256 + tid) / 16 * 128 + (256 + tid) % 16 * 8 % 64 * 2 ^ ((256 + tid) / 16 * 128 + (256 + tid) % 16 * 8 % 64 * 2 >> 7 & 7) << 4))), "l"(index_q + ((row0 + (long long)((256 + tid) / 16)) * 128 + (long long)((256 + tid) % 16 * 8))), "r"((q0 + (256 + tid) / 16 / 64 < seq_len) ? 16 : 0));
    asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
        :: "r"(s_q_addr + (unsigned int)((512 + tid) % 16 * 8 / 64 * 8192 + ((512 + tid) / 16 * 128 + (512 + tid) % 16 * 8 % 64 * 2 ^ ((512 + tid) / 16 * 128 + (512 + tid) % 16 * 8 % 64 * 2 >> 7 & 7) << 4))), "l"(index_q + ((row0 + (long long)((512 + tid) / 16)) * 128 + (long long)((512 + tid) % 16 * 8))), "r"((q0 + (512 + tid) / 16 / 64 < seq_len) ? 16 : 0));
    asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
        :: "r"(s_q_addr + (unsigned int)((768 + tid) % 16 * 8 / 64 * 8192 + ((768 + tid) / 16 * 128 + (768 + tid) % 16 * 8 % 64 * 2 ^ ((768 + tid) / 16 * 128 + (768 + tid) % 16 * 8 % 64 * 2 >> 7 & 7) << 4))), "l"(index_q + ((row0 + (long long)((768 + tid) / 16)) * 128 + (long long)((768 + tid) % 16 * 8))), "r"((q0 + (768 + tid) / 16 / 64 < seq_len) ? 16 : 0));
    if (num_tiles > 0) {
        asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
    ...
    for (int t = 0; t < num_tiles; t++) {
        ...
        int key_b = key_a + 8;
        #pragma unroll
        for (int qi2 = 0; qi2 < 1; qi2++) {
            float part_a = 0.0f;
            float part_b = 0.0f;
            #pragma unroll
            for (int nbq = 0; nbq < 8; nbq++) {
                const int nb = qi2 * 8 + nbq;
                #pragma unroll
                for (int e = 0; e < 2; e++) {
