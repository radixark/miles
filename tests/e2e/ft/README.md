...
grads tolerate max_abs <= 5e-3 — see the dense-mode section below) and metrics (rtol=5e-2).
...
- Post-fault rollouts' dump comparison floors `max_abs <= 5e-3` on the **noisy grad families only** (decoder-layer QK-norms, folded `layer_norm_weight`s, attn/MLP matrices): training data is bitwise-identical, but the degraded commit carries fault-inherent ulp drift into cancellation-dominated near-zero grads. The 2026-09-03 normal-DP versus indep-DP run measured max abs `4.7631e-3` for `linear_qkv`, while real grads sit ~1e-2. Embedding/output/final-norm grads, all activations, and the fault-and-earlier rollouts keep the strict set.
