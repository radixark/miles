...
- **Raw P2P comparison**: every P2P write records SHA-256 over the CPU send buffers of the tensors it carried next to the receiver rank's checksums of those same tensors, taken right after the write and before post-load transformation; the analyzer rule `weight_update_checksum_match` fails the run on any tensor whose two hashes differ. Hashes never gate production publication or add collectives.
