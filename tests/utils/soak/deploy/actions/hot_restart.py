...
from tests.utils.soak.deploy.guard.target_check import read_validated_workloads
...
class HotRestartForm(BaseSoakActionForm):
    ...
    def maybe_create_request(
        ...
        *,
        ...
        if (
            not target.ready
            or (progress := target.finished_rollout_id) is None
            or progress >= self.max_allowed_rollout_id
            or target.saved_iteration is None
        ):
            return None
    ...
    async def execute(
        ...
        await asyncio.to_thread(read_validated_workloads, target)
    ...
    async def _launch(self, *, request: SoakActionRequest, spec: HotRestartLaunchSpec) -> str:
