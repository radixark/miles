...
def due_of_type(events: list[SoakEvent]) -> dict[str, float]:
    landed = {event.request_id for event in events if isinstance(event, SoakActionAppliedEvent)}
    due: dict[str, float] = {}
    for event in events:
        if isinstance(event, SoakScheduleEvent):
            due.update(event.due_of_type)
        elif (
            isinstance(event, SoakActionRequestedEvent)
            and (next_due_at := event.request.next_due_at) is not None
            and event.request.request_id in landed
        ):
            due[event.request.target.kind] = next_due_at
    return due
