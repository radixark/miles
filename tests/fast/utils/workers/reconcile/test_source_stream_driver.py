...
class TestStreamProtocol:
    ...
    async def test_a_reopened_stream_not_opening_with_replace_is_rejected(self):
        """Every replacement stream must describe the whole world before sending incremental events."""
        harness = DriverHarness(source=FakeSource())
        await settle()
        harness.source.emit(replace(make_pod("pod-0", cell="cell-a")))
        await settle()
        ...
        await harness.clock.elapse(1.0)
        await settle()
        affected_before_rejection = list(harness.affected)

        harness.source.emit(UpsertEvent(key="pod-1", obj=make_pod("pod-1", cell="cell-b")))
        await settle()

        assert harness.source.closed_count == 2
        assert harness.source.open_count == 2
        assert harness.affected == affected_before_rejection
        assert [pod.metadata.name for pod in harness.store.get_by_parent("cell-a")] == ["pod-0"]
        assert harness.store.get_by_parent("cell-b") == []

        await harness.clock.elapse(1.0)
        await settle()

        assert harness.source.open_count == 3
        await harness.close()
...
class TestReopen:
    ...
    async def test_a_stream_close_failure_does_not_stop_reopening(self, caplog: pytest.LogCaptureFixture):
        """A cleanup failure is logged without preventing the retry delay or replacement stream."""
        ...
        clock = FakeClock()
        driver = SourceStreamDriver(
            source=source,
            store=ObjectStore(key_map=pod_cell),
            on_affected=lambda _: None,
            retry_delay=3.0,
            clock=clock,
        )
        ...
        assert open_count == 1
        assert "SourceStreamDriver failed to close a source stream" in caplog.text
