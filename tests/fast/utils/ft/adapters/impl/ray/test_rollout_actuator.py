...
class TestRayRolloutActuator:
    ...
    async def test_get_status_failed(self) -> None:
        ...
        actuator = RayRolloutActuator(get_handle=lambda: handle, cell_id="1")
        status = await actuator.get_status()
        assert status == JobStatus.FAILED

    async def test_different_cell_ids_isolated(self) -> None:
        handle = FakeRmHandle()
        act_0 = RayRolloutActuator(get_handle=lambda: handle, cell_id="0")
        act_1 = RayRolloutActuator(get_handle=lambda: handle, cell_id="1")
        await act_0.stop()
        await act_1.stop()
