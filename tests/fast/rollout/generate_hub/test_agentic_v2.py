...
@pytest.mark.asyncio
@pytest.mark.parametrize("use_v2", [True, False], ids=["v2", "v1"])
...
@pytest.mark.asyncio
async def test_infra_abort_cause_survives_a_failed_collection(monkeypatch):
    """One outage can abort the agent and break the collection; the metric must still name the agent's cause."""
    tracer = _Tracer(error=httpx.ConnectError("session server unreachable"))

    async def aborting_agent(**kwargs):
        raise InfraAbort("ServerUnreachable", "env server unreachable")
    ...
    assert output.samples[0].status == Sample.Status.ABORTED
    assert output.samples[0].metadata["exit_status"] == "ServerUnreachable"
