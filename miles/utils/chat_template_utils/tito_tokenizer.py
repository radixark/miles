...
class TITOTokenizer:
    ...
    def _split_appended_segments(self, appended_messages: list[dict[str, Any]]) -> list[list[dict[str, Any]]]:
        ...
        while i < len(appended_messages):
            ...
            if role in {"user", "system", "assistant"}:
                # Assistant-role appends arise when the agent layer inserts a
                # non-generated assistant turn between tool calls (e.g.,
                # terminus-2 self-reflection / planning turns). These are not
                # produced by the inference engine, so they need to be
                # incrementally tokenized the same way user/system single
                # messages are.
    ...
    def _tokenize_assistant_segment(
        ...
        # Assistant-role single-message appends use [dummy_system, dummy_user]
        # as the synthetic context so chat templates render the assistant-turn
        # boundary tokens correctly (most templates require an assistant turn
        # to follow a user or tool turn, never system directly).
        return self._tokenize_rendered_suffix(
    ...
    def tokenize_additional_non_assistant(
        ...
        Handles tool responses, user, system, and agent-layer-inserted
        assistant messages (e.g. terminus-2 self-reflection turns between
        tool calls).  Assistant messages produced by the inference engine are
        NOT passed through this path — they arrive as pretokenized IDs and
        are merged directly.  Validates that *new_messages* is an
        append-only extension of *old_messages* via
        ``assert_messages_append_only_with_allowed_role``.

        The name is retained for backward compatibility; the semantics are
        "tokenize appended messages that were not generated by the inference
        engine."  Agent-layer assistant turns fall into this category.
        ...
        # Incremental appended content is assembled segment-by-segment using
        # the smallest synthetic context that preserves each role's boundary
        # tokens.
