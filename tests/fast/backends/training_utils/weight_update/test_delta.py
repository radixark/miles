...
class TestFirstPublication:
    def test_cold_sync_encodes_the_current_model_after_loading_the_canonical_base(self) -> None:
        """A restored trainer publishes its real weights on the first sync."""
        protocol = UpdateWeightFromDiskDelta.__new__(UpdateWeightFromDiskDelta)
        protocol._baseline_captured = False
        protocol._snapshot_version = 0
        capture = MagicMock()
        encode = MagicMock()
        protocol._capture_baseline = capture
        protocol._begin_encode = encode
        iterator = object()

        assert protocol.begin_sync(12, iterator)

        capture.assert_called_once_with(iterator)
        encode.assert_called_once_with(1)
        assert protocol._baseline_captured


class TestRestartedDeltaStream:
    def test_a_new_trainer_uses_a_fresh_receiver_checkpoint_directory(self, tmp_path: Path) -> None:
        """Restarted transport sequence zero cannot reuse an already advanced receiver cache."""
        args = Namespace(
            update_weight_disk_dir=str(tmp_path / "delta"),
            update_weight_local_checkpoint_dir="/local/ckpt",
            update_weight_delta_encoding="xor",
        ...
        first = UpdateWeightFromDiskDelta(args)
        restarted = UpdateWeightFromDiskDelta(args)

        assert first._local_checkpoint_dir != restarted._local_checkpoint_dir
        assert Path(first._local_checkpoint_dir).parent == Path("/local/ckpt")
        assert Path(restarted._local_checkpoint_dir).parent == Path("/local/ckpt")
...
class TestDeltaPublicationLifecycle:
    def test_repeat_jump_and_rewind_versions_preserve_the_delta_chain(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            for version in [8, 8, 15, 2]:
                weights = torch.tensor([version, 1, 2, 3], dtype=torch.uint8)
                assert protocol.begin_sync(version, lambda weights=weights, **kwargs: [[("weight", weights)]])
