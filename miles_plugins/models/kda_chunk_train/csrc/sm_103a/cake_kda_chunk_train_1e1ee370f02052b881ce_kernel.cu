...
#define SMEM_HS_STAGE_BYTES 8192
#define SMEM_HS_STRIDE 8192
#define SMEM_HS_MN_OFF 66560
#define SMEM_HS_MN_STAGE_BYTES 8192
#define SMEM_HS_MN_STRIDE 8192
#define SMEM_BV_OFF 74752
#define SMEM_BV_STAGE_BYTES 4096
#define SMEM_BV_STRIDE 4096
#define SMEM_BV_MN_OFF 74752
#define SMEM_BV_MN_STAGE_BYTES 4096
#define SMEM_BV_MN_STRIDE 4096
...
kernel_cake_kda_chunk_train_1e1ee370f02052b881ce(CakeTensorMap const* w_tma, CakeTensorMap const* kg_tma, __nv_bfloat16* __restrict__ u, float* __restrict__ gk, __nv_bfloat16* __restrict__ h_out, __nv_bfloat16* __restrict__ v_new, int num_heads, int* __restrict__ seq_chunk_start)
{
    ...
    __nv_bfloat16* bv = reinterpret_cast<__nv_bfloat16*>(smem_raw + 74752);
    const int bv_addr = smem + 74752;
    __nv_bfloat16* bv_mn = reinterpret_cast<__nv_bfloat16*>(smem_raw + 74752);
    const int bv_mn_addr = smem + 74752;
    ...
    const int tmem_acc_vp = taddr + 32;
    ...
    if (warp <= 3) {
        { // compute_main
            ...
            #pragma unroll 1
            for (int c = 0; c < nc; c++) {
                int h_base = (((c0 + c) * num_heads + head) * 128 + k_row) * 128 + vbase;
                #pragma unroll
                for (int blk_1 = 0; blk_1 < 2; blk_1++) {
                    ...
                    #pragma unroll
                    for (int q = 0; q < 2; q++) {
                        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((hs_addr + (unsigned int)((blk_1 * 16 + q * 8) / 32 * 8192 + k_row * 64 + (blk_1 * 16 + q * 8) % 32 * 2 ^ ((blk_1 * 16 + q * 8) / 32 * 8192 + k_row * 64 + (blk_1 * 16 + q * 8) % 32 * 2 >> 7 & 3) << 4))), "r"(_tmem_load_0_bf16[q * 4]), "r"(_tmem_load_0_bf16[q * 4 + 1]), "r"(_tmem_load_0_bf16[q * 4 + 2]), "r"(_tmem_load_0_bf16[q * 4 + 3]) : "memory");
                    }
                }
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                ...
                for (int blk_2 = 0; blk_2 < 1; blk_2++) {
                    ...
                    asm volatile(
                        ...
                        : "r"(taddr + 32 + (unsigned int)vp_row_base + (unsigned int)(blk_2 * 32)));
                    asm volatile("tcgen05.wait::ld.sync.aligned;");
                    ...
                    for (int q_1 = 0; q_1 < 2; q_1++) {
                        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((bv_addr + (unsigned int)((col + q_1 * 8) / 32 * 4096 + t_row * 64 + (col + q_1 * 8) % 32 * 2 ^ ((col + q_1 * 8) / 32 * 4096 + t_row * 64 + (col + q_1 * 8) % 32 * 2 >> 7 & 3) << 4))), "r"(bvv_bf16[q_1 * 4]), "r"(bvv_bf16[q_1 * 4 + 1]), "r"(bvv_bf16[q_1 * 4 + 2]), "r"(bvv_bf16[q_1 * 4 + 3]) : "memory");
                    }
                }
        ...
        { // mma_main
            ...
            if (elect_sync()) {
                ...
                for (int c_2 = 0; c_2 < nc_2; c_2++) {
                    ...
                    int _mma_b_lo_0 = (((hs_mn_addr) >> 4) & 0x3FFF) | 0x2000000;
                    asm volatile(
                    "{\n\t"
                    ".reg .pred p0, p1;\n\t"
                    ...
                    int _mma_b_lo_1 = (((bv_mn_addr) >> 4) & 0x3FFF) | 0x1000000;
                    asm volatile(
                    "{\n\t"
                    ".reg .pred p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
