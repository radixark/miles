...
- `miles/utils/transformers_patch.py` now mirrors the target SGLang v0.5.12 config registry by registering DeepSeek V3-family aliases with Transformers. The removed private helper `_load_deepseek_temp_model` is not present in `sglang-miles-v0.5.12`.
