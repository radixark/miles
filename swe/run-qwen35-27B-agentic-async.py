...
Usage:
    ...
    python run-glm47-flash-agentic-async.py --num-nodes 4 --train-num-nodes 2
    python run-glm47-flash-agentic-async.py --num-nodes 8 \\
...
class ScriptArgs(U.ExecuteTrainConfig):
    ...
    agent_max_concurrent: int = os.environ.get("AGENT_MAX_CONCURRENT", "8")
...
def execute(args: ScriptArgs):
    ...
    perf_args = (
        f"--tensor-model-parallel-size 2 "
        "--sequence-parallel "
    ...
    sglang_args = (
        ...
        # "--sglang-reasoning-parser qwen3 " # 处理tool call 出现在reasoning content里的情况，
        "--use-sglang-tool-choice-required "
        "--sglang-grammar-backend xgrammar " # 配合tool choice使用
        "--use-miles-router "
