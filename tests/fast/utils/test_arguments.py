...
class TestSampleOwnershipCheckArguments:
    ...
    def test_supported_runs_get_a_run_specific_event_directory_without_enabling_gpu_witness(self) -> None:
        """The default checker records all evidence even when no debug or checkpoint directory was requested."""
        args = self._parse(["--run-uuid", "0123456789abcdef"])

        miles_validate_args(args)

        assert args.sample_ownership_check is True
    ...
    def test_the_checker_can_be_disabled_explicitly(self) -> None:
        ...
        args = self._parse(["--no-sample-ownership-check"])

        miles_validate_args(args)

        assert args.sample_ownership_check is False
        assert args.enable_witness is False
    ...
    @pytest.mark.parametrize(
        ("flag", "value"),
        [
            ("--sample-ownership-grace-period-seconds", "-1"),
            ("--sample-ownership-check-interval-seconds", "0"),
            ("--sample-ownership-check-timeout-seconds", "0"),
        ],
    )
