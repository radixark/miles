...
async def run_tenant(index: int, args, rows_by_step) -> dict:
    ...
    started = time.time()
    # the first rollout samples the freshly created adapter (B = 0, so it is the base model);
    # base-model sampling through the multi-LoRA router answered 503, so publish first
