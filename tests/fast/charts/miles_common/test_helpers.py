...
kind: Pod
metadata:
  name: {{ include "miles-common.fullname" . }}
  labels:
...
class TestNaming:
    def test_the_release_name_carries_the_chart_name(self, consumer):
        ...
        assert render(consumer, RELEASE_NAME)["metadata"]["name"] == f"{RELEASE_NAME}-consumer"
        assert render(consumer, f"{RELEASE_NAME}-consumer")["metadata"]["name"] == f"{RELEASE_NAME}-consumer"

    def test_a_name_stays_inside_the_label_budget(self, consumer):
