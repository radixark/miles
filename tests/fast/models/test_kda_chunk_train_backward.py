...
the reference forward; the backward under test is :func:`miles_plugins.models.kda_chunk_train.chunk_kda_backward`.
"""

import pytest
import torch

fla = pytest.importorskip("fla", reason="flash-linear-attention (reference) is not installed")
...
from miles_plugins.models.kda_chunk_train import chunk_kda_backward  # noqa: E402
...
def _inputs(batch, seq_len, heads, value_heads, *, seed, packed=False):
    ...
    A_log = torch.log(rand(value_heads) + 1.0)  # model-init regime: decay rates in [1, 2)
...
def _reference(inp, *, beta_is_logit=True):
    leaves = {
        n: inp[n].detach().clone().requires_grad_(True) for n in ("q", "k", "v", "g", "beta", "A_log", "dt_bias")
    }
...
def _saved(inp, *, beta_is_logit=True):
    with torch.no_grad():
        ...
        beta = fused_beta_sigmoid_fwd(inp["beta"], 1.0) if beta_is_logit else inp["beta"].float().sigmoid()
        cu = inp["cu_seqlens"]
        chunk_indices = None if cu is None else prepare_chunk_indices(cu, CHUNK, cu_seqlens_cpu=cu.cpu())
        out = chunk_kda_fwd(
            q=q_norm,
            k=k_norm,
            v=inp["v"],
...
def test_matches_reference_chunk_kda(batch, seq_len, heads, value_heads, packed):
    _requires_blackwell()
    inp = _inputs(batch, seq_len, heads, value_heads, seed=460_000 + seq_len, packed=packed)
    ref = _reference(inp)
...
def test_matches_reference_with_post_sigmoid_beta(batch, seq_len, heads, value_heads, packed):
    ...
    _requires_blackwell()
    inp = _inputs(batch, seq_len, heads, value_heads, seed=462_000 + seq_len, packed=packed)
