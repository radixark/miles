...
- Raise `miles.rollout.agentic.agent_function.InfraAbort(exit_status)` to
  discard the sample. Its group is dropped before any dynamic-sampling filter
  runs, and the drop is counted in `rollout/aborted/drop_<exit_status>`.
