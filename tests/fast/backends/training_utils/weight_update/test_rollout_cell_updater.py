...
def _args() -> Namespace:
    return Namespace(update_weight_engine_request_timeout=10.0)
...
class TestSubmitClientCall:
    ...
    def test_a_client_call_reaches_the_engine_and_hands_back_its_answer(self) -> None:
        ...
        updater = _RolloutCellUpdater(args=_args(), cell_id="cell-a", api_client=client)
    ...
    def test_an_updater_talks_only_to_the_engine_of_its_own_cell(self) -> None:
        ...
        updater = _RolloutCellUpdater(args=_args(), cell_id="cell-first", api_client=first)
...
class TestCreateRolloutCellUpdaters:
    ...
    def test_every_rollout_engine_gets_an_updater_keyed_by_its_cell_id(self) -> None:
        ...
        updaters = create_rollout_cell_updaters(_args(), clients, ["cell-a", "cell-b"])
    ...
    def test_each_updater_is_paired_with_the_engine_at_the_same_position(self) -> None:
        ...
        updaters = create_rollout_cell_updaters(_args(), [first, second], ["cell-a", "cell-b"])
    ...
    def test_metadata_that_describes_a_different_set_of_engines_is_rejected(self) -> None:
        ...
        with pytest.raises(ValueError):
            create_rollout_cell_updaters(_args(), [_FakeApiClient("a"), _FakeApiClient("b")], ["cell-a"])
...
class TestErroredCellIsLeftAlone:
    ...
    def test_a_fresh_cell_is_not_errored(self) -> None:
        ...
        updater = _RolloutCellUpdater(args=_args(), cell_id="cell-a", api_client=_FakeApiClient("a"))
    ...
    def test_marking_is_confined_to_the_cell_it_was_called_on(self) -> None:
        ...
        first = _RolloutCellUpdater(args=_args(), cell_id="cell-a", api_client=_FakeApiClient("a"))
        second = _RolloutCellUpdater(args=_args(), cell_id="cell-b", api_client=_FakeApiClient("b"))
...
class _HangingApiClient:
    def __init__(self) -> None:
        self.entered = threading.Event()

    def __getattr__(self, name: str):
        async def method(**_kwargs: Any) -> Any:
            self.entered.set()
...
def _make_updater(api_client: Any, *, cell_id: str = "cell-0", engine_request_timeout: float = 10.0):
    return _RolloutCellUpdater(
        args=Namespace(update_weight_engine_request_timeout=engine_request_timeout),
        cell_id=cell_id,
        api_client=api_client,
    )


class TestEngineRequestDeadline:
    """Every request to a rollout engine is bounded, so one hung engine cannot stall the trainer."""

    def test_a_request_that_never_returns_gives_the_cell_up(self) -> None:
        """A hung engine must not hold the whole trainer past its own weight-update deadline."""
        client = _HangingApiClient()
        updater = _make_updater(client, engine_request_timeout=0.05)

        result = updater.submit_client_call("get_server_info").result(timeout=10)

        assert client.entered.wait(timeout=10)
        assert result is None
        assert updater.is_errored

    def test_the_deadline_comes_from_the_configured_engine_request_timeout(self) -> None:
        """--update-weight-engine-request-timeout is what bounds a single engine request."""
        recorded: list[float | None] = []
        original_wait_for = asyncio.wait_for

        async def recording_wait_for(request, timeout):
            recorded.append(timeout)
            return await original_wait_for(request, timeout)

        updater = _make_updater(_RecordingApiClient(), engine_request_timeout=12.5)
        with pytest.MonkeyPatch.context() as monkeypatch:
            monkeypatch.setattr(asyncio, "wait_for", recording_wait_for)
            updater.submit_client_call("flush_cache").result(timeout=10)
