...
def _make_event(
    ...
    return TrainEngineLocalWeightChecksumEvent(
        timestamp=_FIXED_TS,
        source=TrainProcessIdentity(component="actor", cell_id=cell_id, rank_within_cell=rank_within_cell),
...
class TestCheck:
    def test_matching_replicas_no_mismatches(self) -> None:
        events = [
            _make_event(rollout_id=0, cell_id="trainer-actor-0", param_hashes={"pp0.weight": "aaa"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-1", param_hashes={"pp0.weight": "aaa"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-2", param_hashes={"pp0.weight": "aaa"}),
        ]
        assert check(events) == []

    def test_param_hash_mismatch_detected(self) -> None:
        events = [
            _make_event(rollout_id=5, cell_id="trainer-actor-0", param_hashes={"pp0.weight": "aaa"}),
            _make_event(rollout_id=5, cell_id="trainer-actor-1", param_hashes={"pp0.weight": "zzz"}),
        ]
    ...
    def test_missing_key_in_one_replica_detected(self) -> None:
        events = [
            _make_event(
                rollout_id=0, cell_id="trainer-actor-0", param_hashes={"pp0.weight": "aaa", "pp0.bias": "bbb"}
            ),
            _make_event(rollout_id=0, cell_id="trainer-actor-1", param_hashes={"pp0.weight": "aaa"}),
        ]
        mismatches = check(events)
    ...
    def test_multiple_steps_only_mismatched_step_reported(self) -> None:
        events = [
            # Step 0: match
            _make_event(rollout_id=0, cell_id="trainer-actor-0", param_hashes={"pp0.w": "aaa"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-1", param_hashes={"pp0.w": "aaa"}),
            # Step 1: mismatch
            _make_event(rollout_id=1, cell_id="trainer-actor-0", param_hashes={"pp0.w": "aaa"}),
            _make_event(rollout_id=1, cell_id="trainer-actor-1", param_hashes={"pp0.w": "zzz"}),
            # Step 2: match
            _make_event(rollout_id=2, cell_id="trainer-actor-0", param_hashes={"pp0.w": "aaa"}),
            _make_event(rollout_id=2, cell_id="trainer-actor-1", param_hashes={"pp0.w": "aaa"}),
        ]
        mismatches = check(events)
    ...
    def test_single_replica_no_comparison(self) -> None:
        events = [_make_event(rollout_id=0, cell_id="trainer-actor-0", param_hashes={"pp0.w": "aaa"})]
        assert check(events) == []

    def test_buffer_mismatch_detected(self) -> None:
        events = [
            _make_event(rollout_id=0, cell_id="trainer-actor-0", buffer_hashes={"pp0.running_mean": "aaa"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-1", buffer_hashes={"pp0.running_mean": "bbb"}),
        ]
        mismatches = check(events)
    ...
    def test_optimizer_state_mismatch_detected(self) -> None:
        events = [
            _make_event(
                rollout_id=3, cell_id="trainer-actor-0", optimizer_state_dict={"state": {0: {"exp_avg": "aaa"}}}
            ),
            _make_event(
                rollout_id=3, cell_id="trainer-actor-1", optimizer_state_dict={"state": {0: {"exp_avg": "bbb"}}}
            ),
        ]
        mismatches = check(events)
    ...
    def test_non_tensor_state_mismatch_detected(self) -> None:
        events = [
            _make_event(rollout_id=0, cell_id="trainer-actor-0", optimizer_state_dict={"state": {0: {"step": 10}}}),
            _make_event(rollout_id=0, cell_id="trainer-actor-1", optimizer_state_dict={"state": {0: {"step": 20}}}),
        ]
        mismatches = check(events)
    ...
    def test_different_ranks_not_compared(self) -> None:
        ...
        events = [
            _make_event(rollout_id=0, cell_id="trainer-actor-0", rank_within_cell=0, param_hashes={"pp0.w": "aaa"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-0", rank_within_cell=1, param_hashes={"pp1.w": "bbb"}),
        ]
        assert check(events) == []
    ...
    def test_same_rank_different_cells_mismatch(self) -> None:
        ...
        events = [
            _make_event(rollout_id=0, cell_id="trainer-actor-0", rank_within_cell=0, param_hashes={"pp0.w": "aaa"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-1", rank_within_cell=0, param_hashes={"pp0.w": "zzz"}),
        ]
        mismatches = check(events)
        assert len(mismatches) >= 1
    ...
    def test_same_rank_different_cells_match(self) -> None:
        ...
        events = [
            _make_event(rollout_id=0, cell_id="trainer-actor-0", rank_within_cell=0, param_hashes={"pp0.w": "aaa"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-1", rank_within_cell=0, param_hashes={"pp0.w": "aaa"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-0", rank_within_cell=1, param_hashes={"pp1.w": "bbb"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-1", rank_within_cell=1, param_hashes={"pp1.w": "bbb"}),
        ]
        assert check(events) == []
    ...
    def test_multi_rank_multi_cell_only_mismatched_rank_reported(self) -> None:
        ...
        events = [
            ...
            # rank 1: mismatch
            _make_event(rollout_id=0, cell_id="trainer-actor-0", rank_within_cell=1, param_hashes={"pp1.w": "bbb"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-1", rank_within_cell=1, param_hashes={"pp1.w": "zzz"}),
        ]
        mismatches = check(events)
        assert len(mismatches) >= 1
    ...
    def test_three_cells_first_vs_rest(self) -> None:
        ...
        events = [
            _make_event(rollout_id=0, cell_id="trainer-actor-0", param_hashes={"w": "aaa"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-1", param_hashes={"w": "aaa"}),
            _make_event(rollout_id=0, cell_id="trainer-actor-2", param_hashes={"w": "bbb"}),
        ]
        mismatches = check(events)
        assert len(mismatches) >= 1
...
class TestFlattenEvent:
    def test_excludes_metadata_fields(self) -> None:
        event = _make_event(rollout_id=0, cell_id="trainer-actor-0", param_hashes={"pp0.w": "aaa"})
    ...
    def test_includes_optimizer_hashes(self) -> None:
        event = _make_event(
            rollout_id=0, cell_id="trainer-actor-0", optimizer_state_dict={"state": {0: {"exp_avg": "hash1"}}}
        )
        flat = _flatten_event(event)
