...
class DeploymentObserver(SoakObserver):
    ...
    async def observe(self) -> SoakObservationEvent:
        ...
        snapshot, uninstall_job_uid, progress = await asyncio.gather(
            self._read_cluster(),
        ...
        targets = self._create_targets(
            snapshot=snapshot, progress=progress, uninstall_job_uid=uninstall_job_uid, errors=errors
        )
    ...
    async def _read_cluster(self) -> ClusterSnapshot:
        ...
        *,
        ...
        return [
            DeploymentTarget(
                ...
                workload_uids={one.name: one.uid for one in snapshot.workloads},
                ...
                state_file=snapshot.orchestrator_state_file,
