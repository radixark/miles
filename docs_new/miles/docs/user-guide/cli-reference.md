...
| `--placement-group-strategy` | str | `STRICT_PACK` | Ray placement-group strategy. |
...
| `--p2p-weight-sync-retries` | int | `3` | Retry count before falling back to broadcast. |

## Async / partial rollout

| Flag | Type | Default | Notes |
|---|---|---|---|
| `--fully-async-rollout` | flag | off | Continuous background worker. |
...
| `--debug-determinism` | flag | off | Log per-step hashes. |
| `--debug-weight-sync` | int | `0` | Verbose P2P transfer logs. |
