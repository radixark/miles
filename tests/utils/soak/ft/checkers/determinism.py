...
from tests.utils.soak.core.views import published_weight_updates
...
def assert_deterministic_environment(
    ...
    *,
    ...
    updated = {
        (cell_id, result.snapshot_cell_id_to_hashes[cell_id])
        for result in published_weight_updates(events).values()
        for cell_id in result.updated_cell_ids
    }
    missing = updated - {(report.cell_id, report.workers_hash) for report in engine_reports}
