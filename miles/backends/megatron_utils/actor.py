...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def log_current_cpu_witness(self, rollout_id: int, cohort_id: str) -> dict[str, object] | None:
        ...
        if not is_local_replica_megatron_main_rank():
            return None
        return make_current_cpu_witness_payload(
            self.model,
            rollout_id=rollout_id,
            cohort_id=cohort_id,
    ...
    def _get_actor_weights(self):
        ...
        if self.args.colocate or self._active_model_tag != "actor":
            return {
                ...
                if not ModelCompanionUtils.is_parameter_name(name)
            }
    ...
    def update_weights(self, info: UpdatableEngines) -> WeightUpdateReport:
        ...
        with torch_memory_saver.disable() if self.args.offload_train else nullcontext():
            ...
            report = self.weight_updater.update_weights(weight_version=self._actor_weight_version())
            ...
            if (
                ...
                and self.weight_updater.protocol.inference_cell_health is None
            ):
