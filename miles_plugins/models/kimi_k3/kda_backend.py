...
atomics, so repeated backward passes on identical inputs are bit-identical). Calls that
kernel does not cover fall back to FLA's backward for that call.
"""
...
def packed_lengths_supported(cu_seqlens: torch.Tensor | None, seq_len: int) -> bool:
    """The deterministic backward covers one fixed length, or packed sequences of one shared length, in 128-multiples."""
...
def deterministic_backward_applies(
    *,
    ...
    fixed or equal-length packed sequences in multiples of 128)."""
    return (
        cp_context is None
        ...
        and packed_lengths_supported(cu_seqlens, seq_len)
    )
...
def _warn_fallback_once(reason: str) -> None:
    ...
    warnings.warn(
        ...
        "the deterministic chunked backward covers SM100a/SM103a, K = V = 128, no context parallelism, "
        "and fixed or equal-length packed sequences whose length is a multiple of 128.",
        stacklevel=3,
    )
