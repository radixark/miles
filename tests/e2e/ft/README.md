...
Architecture (external fault injection, not inside the training loop):
  ...
  2. A background daemon thread iterates every 2s:
     ...
     d. A due kind needs at least two listed cells. Actors need two healthy donors,
        excluding harmful attempts until a healthy replacement generation appears.
        Serving and weight-update phases do not gate faults; spare donors allow
        overlapping recovery. A lost injection response still reserves its victim.
...
- **A form that leaves its cell running**: `BaseFaultForm.harms_cell` is false for it, so the draw is recorded without charging that cell a recovery; the per-kind exponential schedule paces the next injection.
