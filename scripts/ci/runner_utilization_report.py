#!/usr/bin/env python3
"""
Runner Utilization Report (miles)

Analyzes GitHub Actions job data to calculate runner utilization metrics.
Reports idle time, active time, utilization percentage, and per-job queue-wait
percentiles (P50/P90/P99) per runner label.

Ported from sgl-project/sglang scripts/ci/utils/runner_utilization_report.py
(where the accounting logic is unit-tested); this copy adds the percentile
columns and defaults to this repo.
"""
...
def calculate_utilization(
    ...
    for label in sorted(all_labels):
        ...
        p50_queue = percentile(queue_times, 0.5) or 0
        p90_queue = percentile(queue_times, 0.9) or 0
        p99_queue = percentile(queue_times, 0.99) or 0
        ...
        results.append(
            {
                ...
                "p50_queue_min": p50_queue / 60,
                "p90_queue_min": p90_queue / 60,
                "p99_queue_min": p99_queue / 60,
...
def format_report(
    ...
    lines.extend(
        [
            "| Label | Runners | Jobs | Active (hrs) | Utilization | Avg Queue | P50 | P90 | P99 | Max Queue | Status |",
            "|-------|---------|------|--------------|-------------|-----------|-----|-----|-----|-----------|--------|",
        ]
    )
    for r in results:
        ...
        lines.append(
            ...
            f"{r['avg_queue_min']:.1f}m | "
            f"{r['p50_queue_min']:.1f}m | {r['p90_queue_min']:.1f}m | "
            f"{r['p99_queue_min']:.1f}m | {r['max_queue_min']:.1f}m | "
