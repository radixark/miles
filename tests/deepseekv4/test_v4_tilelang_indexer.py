...
FORWARD_CONFIG_IDS = [
    f"sq{sq}_b{b}_h{h}_d{d}_cr{cr}_top{tk}" for sq, b, h, d, cr, tk in FORWARD_CONFIGS
]
...
def test_indexer_forward_scores(seqlen_q, batch, heads, dim, compress_ratio, topk):
    ...
    from miles_plugins.models.deepseek_v4.ops.tilelang_indexer_fwd import (
...
def test_indexer_forward_topk(seqlen_q, batch, heads, dim, compress_ratio, topk):
    ...
    """
    from miles_plugins.models.deepseek_v4.ops.tilelang_indexer import v4_lighting_indexer
    from miles_plugins.models.deepseek_v4.ops.tilelang_indexer_fwd import (
        _make_causal_cu_seqlens,
        batched_indexer_fwd,
    )
...
BACKWARD_CONFIG_IDS = [
    f"sq{sq}_b{b}_h{h}_d{d}_cr{cr}_top{tk}" for sq, b, h, d, cr, tk in BACKWARD_CONFIGS
]
...
def test_indexer_backward(seqlen_q, batch, heads, dim, compress_ratio, topk):
    ...
    ref_grad_q, ref_grad_w, ref_grad_k = ref_indexer_backward_dense(
        q, k, weights, compress_ratio, topk, tl_topk
    )
