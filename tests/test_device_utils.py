"""Tests for miles.utils.device_utils module."""

import pytest
import torch

from miles.utils.device_utils import (
    get_device_property,
    get_device_info,
    get_all_device_info,
    get_gpu_memory_info,
...
class TestGetDeviceProperty:
    ...
    def test_valid_properties(self):
        ...
        total_memory_gb = get_device_property(0, 'total_memory_gb')
        assert isinstance(total_memory_gb, float)
...
class TestGetDeviceInfo:
    ...
    def test_invalid_device(self):
        """Test with invalid device index."""
        result = get_device_info(999)
        assert result['name'] == 'Invalid Device'
        assert result['arch'] == 'invalid'
    ...
    def test_keys_exist(self):
        ...
        result = get_device_info(0)
        expected_keys = ['name', 'compute_cap', 'total_memory_gb', 
                        'multiprocessor_count', 'max_threads_per_block', 'arch']
        for key in expected_keys:
            assert key in result


class TestGetAllDeviceInfo:
    """Tests for get_all_device_info function."""
    
    def test_empty_without_cuda(self):
        """Test that it returns empty list without CUDA."""
        result = get_all_device_info()
        if not torch.cuda.is_available():
            assert result == []
...
class TestGetAllGpuMemoryInfo:
    """Tests for get_all_gpu_memory_info function."""
    
    def test_empty_without_cuda(self):
        """Test that it returns empty dict without CUDA."""
        result = get_all_gpu_memory_info()
        assert result == {}
...
class TestIsHardwareSupported:
    """Tests for is_hardware_supported function."""
    
    def test_no_cuda(self):
        """Test when CUDA is not available."""
        result = is_hardware_supported()
        assert result == (False, "CUDA is not available")
...
class TestGetNcclBackendInfo:
    """Tests for get_nccl_backend_info function."""
    
    def test_returns_dict(self):
        """Test that function returns a dictionary."""
        result = get_nccl_backend_info()
        assert isinstance(result, dict)
        assert 'nccl_version' in result
        assert 'debug_level' in result
...
class TestEstimateModelMemoryRequirements:
    ...
    def test_with_full_config(self):
        """Test estimation with full MoE configuration."""
        est = estimate_model_memory_requirements(
            num_parameters=400_000_000_000,
            precision="fp16",
            num_layers=128,
        ...
        assert "moe_overhead_gb" in est
    
    def test_400b_moe_estimate(self):
        """Test 400B MoE model memory estimation."""
        est = estimate_model_memory_requirements(
            num_parameters=400_000_000_000,
            precision="fp16",
            num_layers=128,
        ...
        # 400B params * 2 bytes = 800 GB for fp16
        assert est["weights_gb"] == 800.0
        assert "total_estimate_gb" in est
        assert est["total_estimate_gb"] > est["weights_gb"]
