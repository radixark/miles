...
- Uses fixed 128-sample microbatches, one nominal DP2 shard per microbatch. The normal-DP baseline computes one such block on each DP rank; the degraded DP1 retry computes the same two block-sized units sequentially instead of dynamically repacking all 256 samples into a different accumulation schedule.
