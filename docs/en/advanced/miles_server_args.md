...
| `--colocate` | Deploy training and rollout on the same GPUs. Enables `--offload-train` and `--offload-rollout`. **Tip:** When enabled, it is highly recommended to set `--sglang-mem-fraction-static` to `0.8` to ensure Megatron has enough memory to initialize before weights are offloaded to CPU. | `False` | bool flag (set to enable) | Miles Native |
