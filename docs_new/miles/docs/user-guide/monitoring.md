...
| any other path | GET / POST / PUT / DELETE | Proxied to a selected SGLang worker — e.g. `/generate`, `/v1/chat/completions`, `/health`. Implemented as FastAPI's catch-all route in `miles/router/router.py`. |
