...
Label commands execute on a command-App token minted with `Issues: write` plus `Pull requests: write`: a label added with `GITHUB_TOKEN` would never fire the `pull_request(labeled)` CI workflows, and the label lands on a pull request, whose issues-API mutations GitHub gates on the pull-requests scope. `Pull requests: write` also lets its holder submit reviews, including approvals. The handler never calls a review endpoint, and the token is revoked when the label-command job ends. Branch protection on `main` requires one approving review; for changes under code-owned paths such as `.github/workflows/` and `miles/` that review must come from a code owner, which an App cannot be, while a pull request touching only unowned paths could be satisfied by an App approval. The mitigation is therefore the fixed handler code and the job-scoped token, not the review rule. Neither token reaches the jobs that execute PR code, and the App private key never leaves the label path.
