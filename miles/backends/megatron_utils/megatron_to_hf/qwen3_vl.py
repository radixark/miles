...
def convert_qwen3vl_to_hf(args, name, param):
    ...
    match = re.match(r"module\.module\.vision_model\.decoder\.deepstack_merger_list\.(\d+)\.(.+)", name)
    if match:
        item_idx, rest = match.groups()
        base = f"model.visual.deepstack_merger_list.{item_idx}"
    ...
    match = re.match(r"module\.module\.vision_model\.merger\.(.+)", name)
    if match:
        rest = match.group(1)
        if rest == "patch_norm.weight":
            return [("model.visual.merger.norm.weight", param)]
        if rest == "patch_norm.bias":
            return [("model.visual.merger.norm.bias", param)]
        return [(f"model.visual.merger.{rest}", param)]
