...
class SGLangEngine(RayActor):
    ...
    def register_lora_adapter(self, lora_name: str, config_dict: dict, pinned: bool = False):
        return async_utils.run(
            self.api_client.register_lora_adapter(lora_name=lora_name, config_dict=config_dict, pinned=pinned)
        )
    ...
    def begin_weight_update(self, selector: str = "all", sync_base: bool = True):
        return async_utils.run(self.api_client.begin_weight_update(selector=selector, sync_base=sync_base))

    def end_weight_update(self, expected_lora_checksums=None):
        return async_utils.run(self.api_client.end_weight_update(expected_lora_checksums=expected_lora_checksums))
