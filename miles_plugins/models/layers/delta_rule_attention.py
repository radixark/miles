...
over the group-major q/k/v (see ``delta_rule_layout``), the fla kernel chosen by the
...
class _ShardedShortConvolution(ShortConvolution):
    """fla's depthwise conv over this rank's channels. fla indexes with int32, so inputs past 2**31 elements
    (long sequences at low TP) run as channel chunks of at most 2**30; the conv is per channel, so this is
    exact."""
    ...
    def forward(self, x: torch.Tensor, cu_seqlens=None, cp_context=None) -> torch.Tensor:
        ...
        if tokens * x.shape[-1] <= INT32_ELEMENTS:
            return super().forward(x=x, cu_seqlens=cu_seqlens, cp_context=cp_context)[0]
        width = max(_CHANNEL_ALIGN, min(INT32_ELEMENTS, _CHUNK_ELEMENTS) // tokens // _CHANNEL_ALIGN * _CHANNEL_ALIGN)
        chunks = [
            _ContiguousGrad.apply(
                causal_conv1d(
                    x=chunk.contiguous(),
                    weight=weight[:, 0],
                    bias=None,
            ...
            for chunk, weight in zip(x.split(width, dim=-1), self.weight.split(width), strict=True)
        ]
