...
from tests.utils.soak.core.views import project_actions, read_training_events
...
def assert_resizes_follow_schedule(
    ...
    for index, (step, action) in enumerate(zip(schedule, actions, strict=True)):
        ...
        assert request.details == ResizeDetails(
            replicas=step.replicas, at_rollout=step.at_rollout
        ...
        assert (evidence.replicas_before, evidence.replicas_after) == (sizes[index], sizes[index + 1]), (
            f"resize {index} moved {request.target.identity} {evidence.replicas_before} -> "
            f"{evidence.replicas_after}, and the schedule has it {sizes[index]} -> {sizes[index + 1]} ({sizes})"
        )

    print(f"the pool was resized as scheduled: {sizes}")


def read_engine_counts_of_rollout(events: list[SoakEvent], *, dump_dir: Path) -> dict[int, int]:
    counts: dict[int, int] = {}
...
def assert_counts_follow_schedule(
    ...
    *,
    ...
    assert sorted(counts_of_rollout) == list(range(num_rollouts)), (
        f"{subject} is known for rollouts {sorted(counts_of_rollout)}, not for each of the {num_rollouts} rollouts "
        f"exactly once"
    )
    ...
    for rollout_id in range(num_rollouts):
        ...
        assert count == sizes[num_landed], (
            ...
            f"{sizes[num_landed]} there; the whole series is {counts_of_rollout}"
        )

    assert num_landed == len(schedule), (
        f"only {num_landed} of the {len(schedule)} resize(s) ever showed in {subject}: the series "
        f"{counts_of_rollout} never reached {sizes[num_landed + 1]}"
    )
...
def assert_observed_cells(
    events: list[SoakEvent], *, cell_type: str, initial: int, schedule: tuple[ResizeStep, ...]
) -> None:
    assert cell_type in (ROLLOUT_CELL_TYPE, ACTOR_CELL_TYPE), f"no count of observed {cell_type} cells is defined"
    sizes = compute_sizes(initial_replicas=initial, schedule=schedule)
    peak, final = max(sizes), sizes[-1]
    observed = [
        sum(
            1
            for target in event.targets
            if target.kind == cell_type and (target.ready if cell_type == ROLLOUT_CELL_TYPE else target.alive)
        )
        for event in events
