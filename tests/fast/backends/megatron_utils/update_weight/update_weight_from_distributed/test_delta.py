...
_DELTA_MODULE = "miles.backends.megatron_utils.update_weight.update_weight_from_distributed.delta"
...
class TestPostWriteHookConstruction:
    def test_configured_post_write_hook_is_loaded_from_function_registry(self, tmp_path: Path) -> None:
        ...
        with (
            patch("miles.utils.function_registry.load_function", return_value=hook) as load_function,
            patch.object(UpdateWeightFromDiskDelta, "_init_lora"),
        ):
        ...
        assert updater._post_write_hook is hook


class TestReloadEnginesFailureTransitions:
    @staticmethod
    def _make_updater(calls: list[tuple[str, dict]], failing_method: str) -> UpdateWeightFromDiskDelta:
        ...
        updater.rollout_engines = [_RejectingApiClient(calls, failing_method)]
    ...
    def test_reload_engine_failure_stops_before_the_next_lifecycle_phase(
        ...
        updater = self._make_updater(calls, failing_method)
