...
#define SMEM_DQ_S_OFF 103680
#define SMEM_DQ_S_STAGE_BYTES 33792
#define SMEM_DQ_S_STRIDE 33792
#define SMEM_DK_S_OFF 137472
#define SMEM_DK_S_STAGE_BYTES 33792
#define SMEM_DK_S_STRIDE 33792
#define SMEM_RQ_S_OFF 171264
#define SMEM_RQ_S_STAGE_BYTES 256
#define SMEM_RQ_S_STRIDE 256
#define SMEM_RK_S_OFF 171520
#define SMEM_RK_S_STAGE_BYTES 256
#define SMEM_RK_S_STRIDE 256
#define SMEM_DOTQ_S_OFF 171776
#define SMEM_DOTQ_S_STAGE_BYTES 256
#define SMEM_DOTQ_S_STRIDE 256
#define SMEM_DOTK_S_OFF 172032
#define SMEM_DOTK_S_STAGE_BYTES 256
#define SMEM_DOTK_S_STRIDE 256
...
kernel_cake_kda_chunk_train_b24ed89e98812fde2dc1(float* __restrict__ dAqk, float* __restrict__ dAkk, float* __restrict__ gk, __nv_bfloat16* __restrict__ k_e, __nv_bfloat16* __restrict__ q_e, float* __restrict__ beta, float* __restrict__ dq_f, float* __restrict__ dk_f, float* __restrict__ dg_f, float* __restrict__ db_f, float* __restrict__ q_rstd, float* __restrict__ k_rstd, int* __restrict__ chunk_bos, int* __restrict__ chunk_len, __nv_bfloat16* __restrict__ dq_ext, __nv_bfloat16* __restrict__ dk_ext, float* __restrict__ dg_out, float* __restrict__ db_out, int num_heads, int num_qk_heads, int group)
{
    ...
    float* dq_s = reinterpret_cast<float*>(smem_raw + 103680);
    const int dq_s_addr = smem + 103680;
    float* dk_s = reinterpret_cast<float*>(smem_raw + 137472);
    const int dk_s_addr = smem + 137472;
    float* rq_s = reinterpret_cast<float*>(smem_raw + 171264);
    const int rq_s_addr = smem + 171264;
    float* rk_s = reinterpret_cast<float*>(smem_raw + 171520);
    const int rk_s_addr = smem + 171520;
    float* dotq_s = reinterpret_cast<float*>(smem_raw + 171776);
    const int dotq_s_addr = smem + 171776;
    float* dotk_s = reinterpret_cast<float*>(smem_raw + 172032);
    const int dotk_s_addr = smem + 172032;
    ...
    if (tid_0 < 64) {
        ...
        float rk_v = 0.0f;
        if (tid_0 < clen) {
            rq_v = q_rstd[(ext0 + (long long)tid_0) * (long long)num_qk_heads + (long long)qk_head];
            rk_v = k_rstd[(ext0 + (long long)tid_0) * (long long)num_qk_heads + (long long)qk_head];
        }
        rq_s[tid_0] = rq_v;
        rk_s[tid_0] = rk_v;
    }
        ...
        for (int nn_5 = 0; nn_5 < 4; nn_5++) {
            ...
            for (int half = 0; half < 2; half++) {
                ...
                for (int e = 0; e < 2; e++) {
                    ...
                    if (half == 0) {
                        dotq_lo = dotq_lo + dq_o[e] * qt;
                        dotk_lo = dotk_lo + dk_o[e] * kt;
                    } else {
                        dotq_hi = dotq_hi + dq_o[e] * qt;
                        dotk_hi = dotk_hi + dk_o[e] * kt;
                    }
                }
                #pragma unroll
                for (int e_1 = 0; e_1 < 2; e_1++) {
                    dq_s[t_loc * 132 + col0 + e_1] = dq_o[e_1];
                    dk_s[t_loc * 132 + col0 + e_1] = dk_o[e_1];
                }
                {
    ...
    float _shfl_xor_4 = __shfl_xor_sync(0xFFFFFFFF, dotq_lo, 1);
    dotq_lo = dotq_lo + _shfl_xor_4;
    float _shfl_xor_5 = __shfl_xor_sync(0xFFFFFFFF, dotq_lo, 2);
    dotq_lo = dotq_lo + _shfl_xor_5;
    float _shfl_xor_6 = __shfl_xor_sync(0xFFFFFFFF, dotq_hi, 1);
    dotq_hi = dotq_hi + _shfl_xor_6;
    float _shfl_xor_7 = __shfl_xor_sync(0xFFFFFFFF, dotq_hi, 2);
    dotq_hi = dotq_hi + _shfl_xor_7;
    float _shfl_xor_8 = __shfl_xor_sync(0xFFFFFFFF, dotk_lo, 1);
    dotk_lo = dotk_lo + _shfl_xor_8;
    float _shfl_xor_9 = __shfl_xor_sync(0xFFFFFFFF, dotk_lo, 2);
    dotk_lo = dotk_lo + _shfl_xor_9;
    float _shfl_xor_10 = __shfl_xor_sync(0xFFFFFFFF, dotk_hi, 1);
    dotk_hi = dotk_hi + _shfl_xor_10;
    float _shfl_xor_11 = __shfl_xor_sync(0xFFFFFFFF, dotk_hi, 2);
    dotk_hi = dotk_hi + _shfl_xor_11;
    __syncthreads();
    if (tq == 0) {
        if (kb_half == 1) {
            beta_s[r0 + grp] = db_lo;
            beta_s[r0 + grp + 8] = db_hi;
            dotq_s[r0 + grp] = dotq_lo;
            dotq_s[r0 + grp + 8] = dotq_hi;
            dotk_s[r0 + grp] = dotk_lo;
            dotk_s[r0 + grp + 8] = dotk_hi;
        }
    }
    __syncthreads();
    if (tq == 0) {
        if (kb_half == 0) {
            ...
            dotq_s[r0 + grp] = dotq_lo + dotq_s[r0 + grp];
            dotq_s[r0 + grp + 8] = dotq_hi + dotq_s[r0 + grp + 8];
            dotk_s[r0 + grp] = dotk_lo + dotk_s[r0 + grp];
            dotk_s[r0 + grp + 8] = dotk_hi + dotk_s[r0 + grp + 8];
        }
    }
    __syncthreads();
    int d0 = lane * 4;
    ...
    for (int it = 0; it < 8; it++) {
        int t = it * 8 + warp;
        long long erow = ext0 + (long long)t;
        float dyq[4];
        float dyk[4];
        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&dyq[0])), "=r"(*reinterpret_cast<uint32_t*>(&dyq[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&dyq[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&dyq[(0) + 3]))
            : "r"(dq_s_addr + (unsigned int)((t * 132 + d0) * 4)));
        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&dyk[0])), "=r"(*reinterpret_cast<uint32_t*>(&dyk[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&dyk[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&dyk[(0) + 3]))
            : "r"(dk_s_addr + (unsigned int)((t * 132 + d0) * 4)));
        float yq[4];
        float yk[4];
        #pragma unroll
        for (int e_2 = 0; e_2 < 4; e_2++) {
            __nv_bfloat16 _yq_b = q_s[t * 136 + d0 + e_2];
            __nv_bfloat16 _yk_b = k_s[t * 136 + d0 + e_2];
            float yq_e = 0.0f;
            float yk_e = 0.0f;
            if (t < clen) {
                float _cvt_f32_10 = __bfloat162float(_yq_b);
                yq_e = _cvt_f32_10;
                float _cvt_f32_11 = __bfloat162float(_yk_b);
                yk_e = _cvt_f32_11;
            }
            yq[e_2] = yq_e;
            yk[e_2] = yk_e;
        }
        float dot_q = dotq_s[t];
        float dot_k = dotk_s[t];
        float rq = rq_s[t];
        float rk = rk_s[t];
        float oq[4];
        float ok[4];
        ...
        for (int e_3 = 0; e_3 < 4; e_3++) {
            oq[e_3] = dyq[e_3] * rq - dot_q * yq[e_3] * rq;
            ok[e_3] = dyk[e_3] * rk - dot_k * yk[e_3] * rk;
        }
        long long qindex = (erow * (long long)num_qk_heads + (long long)qk_head) * 128 + (long long)d0;
        if (t < clen) {
            {
                uint2 _pk2;
                ...
                *reinterpret_cast<uint2*>(&((__nv_bfloat16*)(dq_ext + qindex))[0]) = _pk2;
            }
            {
                uint2 _pk2;
                __nv_bfloat162* _pk = reinterpret_cast<__nv_bfloat162*>(&_pk2);
                ...
                *reinterpret_cast<uint2*>(&((__nv_bfloat16*)(dk_ext + qindex))[0]) = _pk2;
            }
        }
    }
}
