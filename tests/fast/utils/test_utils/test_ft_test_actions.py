...
from miles.utils.test_utils.ft_test_actions import FTTestActionActorExecutor, FTTestActionGroupExecutor
...
class TestResolveCellId:
    def test_non_negative_index_selects_that_cell(self):
        ...
        assert action.resolve_cell_id(spec_name=_SPEC_NAME, num_cells=3) == "trainer-actor-1"

    def test_negative_index_resolves_to_last_cell(self):
        """resolve_cell_id maps the default -1 to the last cell of the configured fleet."""
        action = FTTestAction(at_rollout=5, action="start_cell_at_end", cell_index=-1)
        assert action.resolve_cell_id(spec_name=_SPEC_NAME, num_cells=3) == "trainer-actor-2"
...
class TestRunAfterStep:
    ...
    async def test_start_cell_after_a_stopped_cell_was_dropped_still_targets_that_cell(self):
        ...
        group = FakeGroup(num_cells=2, alive_cell_ids=["trainer-actor-0"])
        manager = FakeWorkerManager()
    ...
    async def test_start_cell_on_a_single_cell_group_whose_only_cell_was_dropped(self):
        ...
        group = FakeGroup(num_cells=1, alive_cell_ids=[])
        manager = FakeWorkerManager()
