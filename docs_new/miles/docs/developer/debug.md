...
* **Slightly elevated logp on instruct (~0.8 per token).** Almost always a chat-template
  ...
  [chat template verifier](/miles/docs/user-guide/agentic-chat-template).
...
the [Reproducibility recipe](/miles/docs/examples/reproducibility) for the exact flag set
