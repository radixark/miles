...
class TestGetKnownBadNodes:
    def test_recovering_st_returns_bad_node_ids(self) -> None:
        """Previously get_known_bad_nodes inspected the inner RecoveryState
        (EvictingAndRestartingSt.restart.bad_node_ids or
        RealtimeChecksSt.pre_identified_bad_nodes), which returned [] for
        phases like StopTimeDiagnosticsSt or NotifyHumansSt. Now bad_node_ids
        is tracked at RecoveringSt level and always returned correctly.
        """
        from datetime import datetime, timezone
        from miles.utils.ft.controller.state_machines.subsystem.models import RecoveringSt
        from miles.utils.ft.controller.state_machines.subsystem.utils import get_known_bad_nodes
        ...
        assert get_known_bad_nodes(state) == ["node-A", "node-B"]

    def test_recovery_state_fallback_still_works(self) -> None:
        """Calling get_known_bad_nodes with a raw RecoveryState (not RecoveringSt)
        still works for backward compatibility (e.g. status.py).
        """
        from miles.utils.ft.controller.state_machines.subsystem.utils import get_known_bad_nodes
        from miles.utils.ft.controller.state_machines.recovery.models import RealtimeChecksSt

        state = RealtimeChecksSt(pre_identified_bad_nodes=["node-X"])
        assert get_known_bad_nodes(state) == ["node-X"]

    def test_unknown_recovery_state_returns_empty(self) -> None:
        from miles.utils.ft.controller.state_machines.subsystem.utils import get_known_bad_nodes
        from miles.utils.ft.controller.state_machines.recovery.models import RecoveryDoneSt

        assert get_known_bad_nodes(RecoveryDoneSt()) == []
