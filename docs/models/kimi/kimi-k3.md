...
The launcher uses scoped HF targets for the existing native LoRA layout: MLA query/KV down projections, attention output projections, dense/shared MLPs, and routed experts. Omit `language_model.model.layers.*.block_sparse_moe.experts.*.w2` to leave routed-expert down projections frozen. Other target combinations, including the full `attn,mlp` groups, are not yet implemented by the native backend.
