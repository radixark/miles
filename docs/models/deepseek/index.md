---
title: DeepSeek
description: Miles recipes for the DeepSeek family — DeepSeek-V4 Flash (sparse-MLA + DSA indexer) and V3.
---
Miles ships recipes for the DeepSeek family across two generations: **DeepSeek-V4 Flash** introduces sparse multi-head latent attention with a learned indexer and KV compressors (8-node H200), while **V3** remains the canonical 16-node 671 B-parameter recipe (BF16 train + 128×128 block-wise FP8 rollout, DeepEP, DAPO-style dynamic sampling).
...
See the [DeepSeek-V4 Flash](/models/deepseek/deepseek-v4-flash) page for the V4 architecture summary, parallelism layouts, and known workarounds; see the [DeepSeek V3](/models/deepseek/deepseek) page for the V3 flow — FP8 → BF16 conversion, Megatron parallelism layout, and the `scripts/run_deepseek.py` launcher.
