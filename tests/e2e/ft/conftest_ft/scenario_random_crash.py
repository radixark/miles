    ...
    *,
    ...
    return asyncio.run(
        run_soak(
            ...
            seed=seed,
            mean_interval_seconds_of_kind=mean_interval_seconds_of_cell_type,
            expected_counts={"actor": ft_mode.num_cells, "rollout": ft_mode.rollout_num_engines},
            ...
            runner_config=SoakRunnerConfig(tail=create_tail_policy(num_rollout=num_steps)),
            forms=forms,
            event_log=event_log,
            observer=create_cell_observer(
                base_url=base_url,
                cell_types=set(mean_interval_seconds_of_cell_type),
                forms=forms,
                config=config,
            ),
        )
