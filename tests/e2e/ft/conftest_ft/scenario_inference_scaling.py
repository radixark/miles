...
from tests.utils.soak.ft.types import ROLLOUT_CELL_TYPE, ResizeStep
...
def _assert_engines_scaled(events: list[SoakEvent], dump_dir: str) -> None:
    ...
    engine_counts = read_engine_counts_of_rollout(events, dump_dir=Path(dump_dir))
    ...
    assert_counts_follow_schedule(
        ...
        subject="the number of engines a weight update reached",
    )
    assert_counts_follow_schedule(
        ...
        schedule=SCHEDULE,
        num_rollouts=SCALING_NUM_ROLLOUTS,
        subject="the engine gpu count the rollout throughput was normalized by",
    )
