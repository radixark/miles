...
class TestFlagValue:
    ...
    def test_refuses_two_stateful_sets_with_the_requested_name(self) -> None:
        """Matching StatefulSets in distinct namespaces still make the requested command line ambiguous."""
        ...
        with pytest.raises(AssertionError, match=f"holds 2 StatefulSets named {ORCHESTRATOR}"):
            manifest.flag_value("--state-file", stateful_set=ORCHESTRATOR, container="orchestrator")
