...
def _install_adapter(adapter, args, model, optimizer) -> int:
    ...
    matches this deployment's shape; otherwise fresh init at step 0."""
...
def load_adapters(args, model, optimizer, adapters) -> int:
    ...
    for adapter in adapters:
        resume_steps[adapter.name] = _install_adapter(adapter, args, model, optimizer)
    if dist.is_initialized():
        dist.barrier(group=get_gloo_group())
    ...
    if is_first_replica_megatron_main_rank():
        ...
        ray.get(controller.mark_ready.remote(sorted(resume_steps)))
    return len(adapters)
...
def _execute_state_op(op: dict, args, model, optimizer, loaded_adapters, pending_push) -> dict:
    ...
    if kind == "save_state":
        ...
        if not _STATE_TAG.fullmatch(tag):
    ...
    # weights, so the restored adapter re-publishes on the next push.
    pending_push.add(name)
