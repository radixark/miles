...
from tests.utils.soak.ft.checkers.resize import (
    ...
    assert_schedule_leaves_room,
)
from tests.utils.soak.ft.entrypoint import run_cell_soak
from tests.utils.soak.ft.types import ACTOR_CELL_TYPE, POOL_TARGET_KIND, ROLLOUT_CELL_TYPE, CellTarget
...
def compute_scaling_moments(phase: RunPhase) -> tuple[RunMoment, ...]:
    return (RunMoment(at_rollout=2, phase=phase), RunMoment(at_rollout=7, phase=phase))
...
def run_scaling_scenario(
    *,
    ...
    assert_schedule_leaves_room(moments, sizes, initial_replicas=initial_replicas, num_rollouts=SCALING_NUM_ROLLOUTS)
    ...
    injector = asyncio.run(
        run_cell_soak(
            ...
            runner_config=SoakRunnerConfig(
                seed=0,
                target_configs={
                    POOL_TARGET_KIND: SoakTargetConfig(expected_count=1, trigger=MomentTrigger(moments=moments))
                },
            ...
            forms={
                POOL_TARGET_KIND: [
                    ResizePoolForm(
                        ...
                        cell_type=cell_type,
                        sizes=sizes,
                        min_replicas=min(initial_replicas, *sizes),
                        max_replicas=max(initial_replicas, *sizes),
                    )
                ]
            },
        )
    ...
    assert_resizes_follow_schedule(events, sizes=sizes, initial_replicas=initial_replicas)
    assert_observed_cells(
        events,
        cell_type=cell_type,
    ...
    print(f"Scaling test PASSED ({test_name}, rollouts={SCALING_NUM_ROLLOUTS}, moments={moments}, sizes={sizes})")
