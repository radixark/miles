...
kernel_cake_kda_chunk_train_7746fbe47fa5f1b86d36(float* __restrict__ dg_intra, __nv_bfloat16* __restrict__ g_raw, float* __restrict__ db_total, __nv_bfloat16* __restrict__ beta_raw, float* __restrict__ A_log, float* __restrict__ dt_bias, int* __restrict__ chunk_bos, int* __restrict__ chunk_len, __nv_bfloat16* __restrict__ dg_out, __nv_bfloat16* __restrict__ dbeta, float* __restrict__ dA_part, float* __restrict__ dbias_part, int num_heads, float lower_bound)
{
    ...
    for (int u_1 = 0; u_1 < 8; u_1++) {
        int t_u = tgrp * 8 + u_1;
        long long eidx_u = ((ext0 + (long long)t_u) * (long long)num_heads + (long long)head) * 128 + (long long)dim;
        float dyg = offset + dgs[u_1];
        float x = 0.0f;
        if (t_u < clen) {
            x = (float)g_raw[eidx_u] + bias;
        }
    ...
    if (tgrp == 1) {
        if (dim < clen) {
            long long ebeta = (ext0 + (long long)dim) * (long long)num_heads + (long long)head;
            long long ibeta = (row0 + (long long)dim) * (long long)num_heads + (long long)head;
            float _expf_2 = __expf(-(float)beta_raw[ebeta]);
            float _rcp_1 = approx_rcp(1.0f + _expf_2);
            float sb = _rcp_1;
            __nv_bfloat16 _cvt_bf16_1 = __float2bfloat16(db_total[ibeta] * sb * (1.0f - sb));
            dbeta[ebeta] = _cvt_bf16_1;
        }
    }
}
