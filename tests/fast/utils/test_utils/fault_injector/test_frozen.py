...
class TestSleepForeverHook:
    ...
    async def test_matching_step_parks_forever_and_replaces_the_sentinel(
        self,
        configure_hooks: Callable[..., _FaultHookController],
        parkable_script: None,
        tmp_path: Path,
    ...
    @pytest.mark.parametrize(
        "requests,rollout_id", [([_SLEEP], 0), ([_SLEEP], 1), ([_SLEEP], 3), ([_SLEEP], 99), ([], 2)]
    )
    async def test_unmatched_or_absent_plan_returns_without_freezing(
        self,
        configure_hooks: Callable[..., _FaultHookController],
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ...
    async def test_other_owners_do_not_arm_the_orchestrator_freeze(
        self, configure_hooks: Callable[..., _FaultHookController], owner: FaultHookOwner
    ) -> None:
    ...
    def test_orchestrator_requests_cannot_name_a_cell(self) -> None:
        ...
        with pytest.raises(ValidationError, match="outside any cell"):
            FaultHookRequest(
                ...
                target=DeclaredFaultHookTarget(cell_id="trainer-engine-actor-0"),
            )


class TestParkableLoops:
    @pytest.mark.parametrize(
        "script,interval,model_id,message",
        [
    ...
    async def test_unsupported_loop_fails_before_writing_a_sentinel(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
        script: str,
        ...
        with pytest.raises(AssertionError, match=message):
            await SleepForeverAction()(
                context=FaultHookContext(rollout_id=2, trainer_model_id=model_id),
    ...
    async def test_an_unarmed_async_loop_is_not_subject_to_freeze_guards(
        self, configure_hooks: Callable[..., _FaultHookController], monkeypatch: pytest.MonkeyPatch
    ) -> None:
