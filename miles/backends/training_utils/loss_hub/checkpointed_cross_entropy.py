...
def checkpointed_sft_output_processor(
    ...
    **_: Any,
    ...
    # In ordinary (non-true-on-policy) mode this helper only aligns/slices
    # sequence tensors; it neither scales nor upcasts the hidden states.
    if args.true_on_policy_mode:
        raise ValueError("checkpointed SFT output does not support true-on-policy mode")
    response_parts = get_responses(
        hidden_batch_first,
        args=args,
    ...
    log_probs = []
    for hidden_part, target_tokens in response_parts:
