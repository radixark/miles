...
import pytest
from tests.fast.utils.workers.worker_provider.kubernetes.views.test_pod_info import make_pod, make_unlabelled_pod

from miles.utils.workers.reconcile.k8s_api import PodListPage, PodWatchEvent
from miles.utils.workers.rpc.client.handle import RpcWorkerHandle
from miles.utils.workers.worker_provider.kubernetes.helm.labels import DEFAULT_LABEL_KEYS, POOL_LABEL
...
def _run(api, *, ports, worker_classes=None, spec_metas=None, ranks_per_pod=None) -> KubernetesRun:
    worker_classes = worker_classes or {}
    spec_metas = spec_metas or {}
    ranks_per_pod = ranks_per_pod or {}
    return KubernetesRun(
        ...
        pool_ids={
            pool_id: PoolView(
                ...
                ranks_per_pod=ranks_per_pod.get(pool_id, 1),
            )
...
def _relabelled(pool_id):
    pod = make_pod(name="engine-0-0", labels={POOL_LABEL: pool_id})
...
class TestWatchCells:
    ...
    def test_reports_a_cell_that_is_not_alive_yet_as_gone(self):
        ...
        api = FakePodApi(pods=[make_pod(name="engine-0-0"), make_pod(name="engine-0-1", pod_index="1", ready=False)])
        provider = _provider(api)
        reported = []
    ...
    def test_pushes_exactly_what_a_direct_query_would_answer(self):
        ...
        api = FakePodApi(
            pods=[
                make_pod(name="engine-0-0", pod_index="0", annotations={"miles.radixark.io/meta-model_id": "glm"}),
                make_pod(name="engine-0-1", pod_index="1", annotations={"miles.radixark.io/meta-model_id": "glm"}),
            ]
        )
        provider = _provider(api)
        reported = []
...
class TestWatchedPods:
    def test_asks_the_apiserver_only_for_the_pools_it_was_built_for(self):
        ...
        assert all(selector == f"{SELECTOR},{POOL_LABEL} in (engine)" for selector in api.selectors), api.selectors
    ...
    def test_names_every_pool_it_watches_in_one_selector(self):
        ...
        assert api.selectors[0] == f"{SELECTOR},{POOL_LABEL} in (engine,router)"
...
class TestCellInfo:
    def test_orders_the_workers_by_rank(self):
        ...
        api = FakePodApi(
            pods=[
                make_pod(name="engine-0-1", pod_index="1"),
                make_pod(name="engine-0-0", pod_index="0"),
            ]
        )
        provider = _provider(api)
    ...
    def test_names_every_rank_of_a_pod_that_serves_more_than_one(self):
        ...
        api = FakePodApi(pods=[make_pod(name="engine-0-0"), make_pod(name="engine-0-1", pod_index="1")])
    ...
    def test_still_reports_the_pods_themselves_for_the_operations_that_delete_them(self):
        ...
        api = FakePodApi(pods=[make_pod(name="engine-0-1", pod_index="1"), make_pod(name="engine-0-0")])
        provider = _provider(api, ranks_per_pod={"engine": 2})
...
class TestSpecMeta:
    def test_evaluates_the_meta_of_the_spec_for_every_cell_that_was_observed(self):
        """Cells of one pool_id differ by their index alone, so one evaluation would collapse them into one."""
        api = FakePodApi(pods=[make_pod(name="engine-0-0"), make_pod(name="engine-1-0", cell_index="1")])
    ...
    def test_refuses_a_cell_whose_pods_annotate_one_key_differently(self):
        ...
        api = FakePodApi(
            pods=[
                ...
                make_pod(name="engine-0-1", pod_index="1", annotations={"miles.radixark.io/meta-model_id": "qwen"}),
            ]
        )
    ...
    def test_accepts_pods_that_agree_about_a_key(self):
        ...
        api = FakePodApi(
            pods=[
                ...
                make_pod(name="engine-0-1", pod_index="1", annotations=annotations),
            ]
        )
...
class TestGetWorkerInfos:
    def test_orders_the_workers_by_the_rank_label(self):
        ...
        api = FakePodApi(
            pods=[
                make_pod(name="engine-0-2", pod_index="2", pod_ip="10.0.0.3"),
                make_pod(name="engine-0-0", pod_index="0", pod_ip="10.0.0.1"),
                make_pod(name="engine-0-1", pod_index="1", pod_ip="10.0.0.2"),
            ]
        )
    ...
    def test_refuses_a_cell_that_is_missing_a_pod(self):
        ...
        api = FakePodApi(pods=[make_pod(name="engine-0-1", pod_index="1")])
