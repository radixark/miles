...
class EngineEnvReporter:
    ...
    async def report_if_due(
        self, *, cell_id: str, workers_hash: str, server_url: str, api_client: SGLangApiClient
    ) -> None:
        now = time.monotonic()
        ...
        reported = await self._report(
            cell_id=cell_id, workers_hash=workers_hash, server_url=server_url, api_client=api_client
        )
    ...
    async def _report(self, *, cell_id: str, workers_hash: str, server_url: str, api_client: SGLangApiClient) -> bool:
        try:
            ...
            if is_event_logger_initialized():
                get_event_logger().log(
                    EngineEnvReportEvent,
                    {
                        ...
                        "server_info": redact_server_info(server_info),
                    },
                    print_log=False,
                )
            return True
