...
from tests.fast.fixtures.kubernetes_fixtures import install_workers

from miles.utils.function_registry import function_registry
from miles.utils.workers.pod_context import SUBPROCESS_INDEX_ENV_VAR, read_rank_in_pod
from miles.utils.workers.serving import serve_inner
from miles.utils.workers.serving.utils import serve_spec_of
...
def worker_of(worker_argv: list[str]) -> Any:
    spec = serve_spec_of(specs_fn=SPECS_FN, pool_id=POOL_ID, worker_argv=worker_argv)
...
class TestCreateWorker:
    ...
    def test_refuses_a_pool_the_run_does_not_describe(self, registered_functions):
        ...
        with pytest.raises(AssertionError, match="not one spec named"):
            serve_spec_of(specs_fn=SPECS_FN, pool_id="no-such-pool", worker_argv=[])
...
class TestDeferredCapability:
    def test_the_backend_is_built_only_when_a_spec_asks_for_a_provider(
        ...
        monkeypatch.setattr(serve_inner, "parse_args_from_argv", lambda argv: SimpleNamespace(cluster_backend="ray"))
...
class TestRpcPortOfARank:
    def test_the_ranks_of_one_pod_listen_on_different_ports(self, registered_functions):
        """The supervisor runs them all in one network namespace, so a shared port is a bind failure."""
        spec = serve_spec_of(specs_fn=SPECS_FN, pool_id=POOL_ID, worker_argv=[])
        ports = [
            serve_inner._rpc_port_of(spec) + read_rank_in_pod({SUBPROCESS_INDEX_ENV_VAR: str(index)})
            for index in range(4)
        ]

        assert ports == [8000, 8001, 8002, 8003]

    def test_the_first_rank_keeps_the_port_the_address_book_predicts(self, registered_functions):
        """The provider addresses a pod at the spec's static rpc port, which rank zero has to answer on."""
        spec = serve_spec_of(specs_fn=SPECS_FN, pool_id=POOL_ID, worker_argv=[])

        assert serve_inner._rpc_port_of(spec) + read_rank_in_pod({}) == RPC_PORT
