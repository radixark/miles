...
| Array | Governs |
|---|---|
| [`MODEL_ARGS`](/miles/docs/user-guide/argument-groups#model-args) | Architecture constants (layers, hidden size, rotary base, ...) |
| [`CKPT_ARGS`](/miles/docs/user-guide/argument-groups#ckpt-args) | Filesystem paths for the actor / reference / save directory |
| [`ROLLOUT_ARGS`](/miles/docs/user-guide/argument-groups#rollout-args) | Prompt dataset, batch knobs, sampling parameters, reward type |
| [`EVAL_ARGS`](/miles/docs/user-guide/argument-groups#eval-args) | Eval dataset, cadence, sampling overrides for evaluation |
| [`PERF_ARGS`](/miles/docs/user-guide/argument-groups#perf-args) | Parallelism (TP/PP/CP/EP/ETP), recomputation, dynamic batching |
| [`GRPO_ARGS`](/miles/docs/user-guide/argument-groups#grpo-args) | RL algorithm, KL, clipping, entropy bonus, advantage estimator |
| [`OPTIMIZER_ARGS`](/miles/docs/user-guide/argument-groups#optimizer-args) | Learning rate, schedule, weight decay, Adam betas |
| [`SGLANG_ARGS`](/miles/docs/user-guide/argument-groups#sglang-args) | Engine TP, memory fraction, log level, `--sglang-*` passthrough |
...
The three roles — actor, frozen reference, HuggingFace directory — are defined in [Core Concepts](/miles/docs/user-guide/concepts#the-four-objects). Here they map to four flags:
...
These two products must be equal — that's the [four-knob invariant](/miles/docs/user-guide/concepts#the-four-knob-invariant). Set three sides; Miles fills in the fourth. Set all four and Miles validates the equation — inconsistent values abort early.
...
- **`--use-tis` is the numerical safety belt.** Switch it on when rollout and trainer
  ...
  the R3 deep dive in [Rollout Routing Replay (R3)](/miles/docs/advanced/miles-router).
...
See the [Fully Async Rollout example](/miles/docs/examples/fully-async) for the full
...
[Low Precision RL](/miles/docs/advanced/fp8-low-precision). For INT4 quant-aware
training, see [INT4 QAT](/miles/docs/advanced/int4-qat).

---

## Next

- [Configuration](/miles/docs/user-guide/cli-reference) — the same material organized as a flag-by-flag
  reference.
- [Server Arguments](/miles/docs/user-guide/cli-reference) — the complete CLI surface.
- [Customization](/miles/docs/user-guide/customization) — the twenty-plus Python extension points.
- [Training Backends](/miles/docs/user-guide/usage) — Megatron vs FSDP and each one's plumbing.
