...
data_dir = "/fs/nlp-intern/yangchengyi/datasets"
harbor_dir = "/fs/nlp-intern/yangchengyi/harbor/datasets/"
task = "swegym"
...
class ScriptArgs(U.ExecuteTrainConfig):
    ...
    data_name: str = "swegym_qwen3.5-27b-fp8_reward1_submitted_first"
    variant: str = f"{data_name}_{model_name}_agentic_async_debug_session_over_sample"
    ...
    sglang_mem_fraction: float = 0.70
    ...
    # Agentic sampling knobs. Miles consumes these through
    # --rollout-temperature/--rollout-top-p/--rollout-top-k and forwards them
    # to the SGLang-compatible chat completion request.
    ...
    agent_server_url: str = os.environ.get(
        "AGENT_SERVER_URL", "http://172.31.72.29:11000"
    )
    ...
    agent_environment_build_timeout_sec: float = 3000
...
def execute(args: ScriptArgs):
    ...
    rollout_args = (
        ...
        f"--rollout-top-k {args.rollout_top_k} "
        f"--rollout-max-response-len {args.rollout_max_response_len} "
        f"--max-seq-len {args.max_seq_len} "
        f"--over-sampling-batch-size {args.over_sampling_batch_size} "
        "--dynamic-sampling-filter-path miles.rollout.filter_hub.dynamic_sampling_filters.check_no_aborted_or_truncated_and_reward_nonzero_std "
