...
class TestLastBatchReplay:
    async def test_raw_handoff_round_trips_real_event_snapshot_and_replay_pipeline(
        ...
        try:
            ...
            record_data_source_issues(data_source)
            ...
            original_record = executor._record_last_batch

            def record_and_queue_save(*, rollout_id: int, samples: list) -> None:
                original_record(rollout_id=rollout_id, samples=samples)
        ...
        events, _ = event_logger.read_events_strict_from(0)
    ...
    @pytest.mark.parametrize(
        ("stage", "expected_reasons"),
        [
            ("raw", ["trim", "dp_schedule_trim"]),
            ("postprocessed", ["dp_schedule_trim"]),
            ("delivered", []),
        ],
    )
    async def test_checkpoint_replay_preserves_each_processing_stage_and_terminal_events(
        self,
        stage: Literal["raw", "postprocessed", "delivered"],
        expected_reasons: list[str],
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
    ) -> None:
        """Raw, postprocessed, and delivered checkpoints resume through only their remaining real pipeline stages."""
        args = make_args(
            load=str(tmp_path),
            save=str(tmp_path),
            global_batch_size=4,
        ...
        raw = [[make_sample(group_index=group, index=group * 10 + slot) for slot in range(2)] for group in range(1, 4)]
        processed, metadata = postprocess_rollout_data(args, raw, train_parallel_config={"dp_size": 2})
        ...
        replay = LastBatch(
            rollout_id=1,
            samples=raw if stage == "raw" else processed,
            stage=stage,
            metadata={} if stage == "raw" else metadata,
            train_data=train_data if stage == "delivered" else None,
        )
        executor = _make_executor(tmp_path, _CountingRolloutFn())
        executor.args = args
        executor._last_batch = replay
        await executor.save(0)
        resumed = _make_executor(tmp_path, _CountingRolloutFn())
        resumed.args = args
        resumed._rollouts_since_publish_of_model_id = defaultdict(int)
        resumed._sample_ownership_task = None
        resumed._metric_checker = None
        resumed.custom_convert_samples_to_train_data_func = None
        resumed.custom_reward_post_process_func = None
        resumed._train_parallel_configs_of_model_id = {None: {"dp_size": 2}}
        ...
        monkeypatch.setattr(
            "miles.ray.rollout.train_data_conversion.build_dp_schedule",
            lambda *args, **kwargs: ([[0], [1]], [[[0]], [[0]]], 1, 2),
        )
        monkeypatch.setattr("miles.ray.rollout.train_data_conversion.object_store.get_instance", Store)
        event_dir = tmp_path / "active-events"
        set_event_logger(EventLogger(log_dir=event_dir, source=SimpleProcessIdentity(component="rollout_executor")))
        try:
            await resumed.load(0)
            result = await resumed.get(rollout_id=1)
        finally:
            set_event_logger(None)

        reasons = [
            event.reason for event in read_events(event_dir) if isinstance(event, ExplicitlyDroppedSamplesEvent)
        ]
        assert reasons == expected_reasons
        assert result.sample_indices == train_data["sample_indices"]
        assert resumed.generate_rollout.num_calls == 0

    async def test_raw_handoff_save_precedes_parent_postprocessing(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
    ) -> None:
        """A save queued by raw handoff captures the batch before its producer parent resumes postprocessing."""
        args = make_args(load=str(tmp_path), save=str(tmp_path), global_batch_size=1, rewards_normalization=False)
        raw = [[make_sample(group_index=1, index=10)]]

        class AsyncRolloutFn(BaseRolloutFn):
            async def __call__(self, input: RolloutFnTrainInput) -> RolloutFnTrainOutput:
                return RolloutFnTrainOutput(samples=raw)

        rollout_fn = AsyncRolloutFn(RolloutFnConstructorInput(args=args, data_source=_FakeDataSource(tmp_path)))
        executor = _make_executor(tmp_path, _CountingRolloutFn())
        executor.args = args
        executor.use_legacy_rollout_v1 = False
        executor.generate_rollout = rollout_fn
        ...
        get_task = asyncio.create_task(executor.get(rollout_id=1))
        assert await asyncio.to_thread(saved.wait, 5)
        state = torch.load(compute_executor_state_path(tmp_path, rollout_id=0), weights_only=False)
        assert state["last_batch"].stage == "raw"
        await get_task
        ...
        assert [sample.index for sample in replayed] == [10]
