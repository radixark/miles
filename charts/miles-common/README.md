...
- Only `miles-workbench` names objects with `fullname`: it is installed by hand, so it has to
  compute its own name. `miles-run` is generated by the miles launcher and is given every
  object name as data, so it computes none.
