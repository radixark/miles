...
- Uses `--clip-grad 2.0`, above this scenario's observed 1.34 maximum grad norm, so the topology-dependent last-bit difference in distributed grad-norm reduction remains measured but cannot change otherwise equal gradients through active clipping. Tensor gradients are still compared directly, and the optimizer still commits the baseline DP2 and degraded DP1 results.
