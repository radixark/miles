...
import miles.backends.megatron_utils.tinker_backend.checkpoint as tc
from miles.backends.megatron_utils.tinker_backend.checkpoint import (
    FORMAT,
    find_slot_state,
    named_state_dir,
...
class TestSlotStateRoundTrip:
    """A state saved from slot A must restore positionally into slot B,
    re-stamping the slot tag, and a child-count mismatch must be refused
    outright, never partially loaded."""
    ...
    def _round_trip(self, tmp_path, monkeypatch, target_children, ttl_seconds=None):
        adapter = make_adapter(tmp_path)
        adapter.step = 7
