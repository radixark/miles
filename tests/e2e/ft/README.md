...
Mechanism: run_cell_soak with one target kind, "pool": the observer lists the engine pool as a
        ...
        scheduler draws that form every poll; the form fires once the run is in the next step's
        rollout and otherwise declines, and it patches spec.replicas of the pool's
        LeaderWorkerSet. Kubernetes creates or deletes the highest group index, the inference
        controller sees the pod through its watch and adds / removes the cell, the next weight
        update covers the new engine, and the rollout executor re-reads the engine topology
        before every rollout.
Schedule (SCHEDULE = compute_scaling_schedule()): 2 -> 3 engines in rollout 2, 3 -> 2 in
        rollout 7
Timing: the form reads the trainer step-end events the soak feeds into every observation and
        fires while the run is in the scheduled rollout N (any phase: after rollout N - 1 was
        trained and before rollout N is); a rollout N already trained fails the soak, a resize
        never fires late
...
- **Why any phase of the rollout**: an engine removed while it is generating is recovered by the rollout ft path the soaks already cover, so where inside rollout N the resize lands does not matter; the scaling witnesses are about the run following the pool's size.
...
Mechanism: as scenario_inference_scaling - the soak's only target is the pool and its only form
        ...
        LeaderWorkerSet, firing while the run is in a scheduled rollout, whatever it is doing
        there. Kubernetes creates or deletes the highest group index (one 2-GPU pod = one
        cell), the trainer controller sees the pod through its watch and adds / removes the
        cell, and the next train step's refresh heals the new cell into the quorum (checkpoint
        from cell 0) or shrinks the quorum to the survivors.
Schedule (SCHEDULE = compute_scaling_schedule()): 2 -> 3 cells in rollout 2, 3 -> 2 in
        rollout 7
Timing and landing: as scenario_inference_scaling (the scheduled rollout read off the trainer
        step-end events, a resize recovers once the pool is observed ready - every actor cell
        Running and Healthy and exactly as many cells as replicas - LANDING_LAG_ROLLOUTS = 3)
...
Assertions:
  ...
  5. Steps: the number of cells in the final attempt of each TrainGroupStepEndEvent is 2 before
     ...
     taking a step down - except that a cell the shrink removes may report "error" in a final
     attempt inside the shrink's lag window
...
- **Why any phase of the rollout**: a trainer cell removed mid-step is recovered by the train ft path the soaks already cover - the step is retried with the survivors, or kept when the cell failed after the gradients were exchanged - so where inside rollout N the resize lands does not matter; the scaling witnesses are about the run following the pool's size.
- **Why the removed cell may report "error"**: the trainer does not retry a step whose surviving cells all report NORMAL after one cell failed (the gradients were already exchanged), so a shrink that lands late in a step leaves that step's final attempt with the removed cell as "error". Any other cell, or "error" outside the shrink's lag window, still fails the scenario.
