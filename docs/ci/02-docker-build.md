...
| `WHEELS_TAG_X86` / `WHEELS_TAG_ARM64`                                                                  | the two **complete** wheels release tags, the wheels repo's own names — rolling per-(CUDA, arch) sets like `cu130-aarch64`, updated in place (legacy pinned tags like `cu130-x86_64-v0.5.12` still work). In a multi-arch build the Dockerfile **picks one by `TARGETARCH`** (the only per-platform value buildx varies) and installs it **verbatim** — never assembling a tag from parts; cu12-x86 overrides `WHEELS_TAG_X86` |
