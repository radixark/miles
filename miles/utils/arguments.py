    ...
    def add_miles_arguments(parser):
        ...
        def add_rollout_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=30.0,
                help=(
                    "Timeout in seconds for one P2P write to one inference cell, collected on the trainer "
                    "rank that submitted it. Each inference cell has its own write thread, so this bounds "
                    "how long a cell may hold up the bucket stream, not how many writes run at once."
                ),
            )
            return parser
...
def _resolve_sample_ownership_check(args: argparse.Namespace) -> None:
    ...
    if args.ci_test:
        args.sample_ownership_check = True
