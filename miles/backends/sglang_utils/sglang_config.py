...
def starts_inference_engines(args) -> bool:
    """A train-only job starts engines only for a dedicated snapshot-eval fleet."""
    return not args.debug_train_only or args.eval_num_gpus > 0


def _compute_raw_sglang_config(args) -> _RawSglangConfig:
    if not starts_inference_engines(args):
...
def _compute_rollout_offset(args) -> int:
    ...
    if args.debug_rollout_only or args.colocate or not starts_inference_engines(args):
        return 0
