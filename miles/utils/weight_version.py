...
def _assert_sample_reports_prefill_weight_versions(sample: "Sample") -> None:
    for call in sample.weight_versions:
        assert not call.output_start or call.prefill_spans, (
            f"sample index={sample.index} has a generation call whose {call.output_start} prompt tokens carry no "
            f"weight version, while --sglang-enable-prefill-weight-versions is on: the engine reported no prompt "
            f"KV versions. Likely causes are an external engine started without --enable-prefill-weight-versions, "
            f"a PD prefill worker (which does not report them yet), or an engine running with HiCache"
        )


def _assert_span_weight_version_sane(*, sample: "Sample", span: "WeightVersionSpan", what: str) -> None:
    assert span.version != SGLANG_DEFAULT_WEIGHT_VERSION, (
        f"sample index={sample.index} tokens [{span.abs_start}, {span.abs_end}) {what} "
    ...
    assert _NUMERIC_VERSION_PATTERN.fullmatch(span.version), (
        f"sample index={sample.index} tokens [{span.abs_start}, {span.abs_end}) {what} weight version "
