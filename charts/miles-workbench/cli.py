...
class Doctor:
    ...
    def may_delegate_rules_it_does_not_hold(self, role: str) -> bool:
        if not self.can_i("escalate", "roles.rbac.authorization.k8s.io"):
            return False
        if self.can_i("bind", "roles.rbac.authorization.k8s.io"):
            return True
        query = ["auth", "can-i", "bind", f"roles.rbac.authorization.k8s.io/{role}", "-n", self.namespace]
...
def helm_install_command(args: argparse.Namespace) -> list[str]:
    ...
    if args.image_tag:
        command += ["--set", f"image.tag={args.image_tag}"]
    if not args.rbac:
...
def exec_shell(args: argparse.Namespace) -> int:
    return run(
        [
            ...
            "exec",
            "-it",
            f"statefulset/{role_name(args.release)}",
            "-n",
            args.namespace,
            "--",
            *args.command,
