...
def install(monkeypatch: pytest.MonkeyPatch, *, pods: list[Any], ranks_per_pod: int = 1):
...
class TestKubernetesDriverAssembly:
    ...
    def test_one_pod_serving_several_ranks_yields_one_worker_per_rank(self, monkeypatch):
        ...
        provider = installed_cells_provider(install(monkeypatch, pods=cell_pods(1), ranks_per_pod=2))
    ...
    def test_the_rollout_executor_answers_over_rpc_with_no_actor_behind_it(self, monkeypatch: pytest.MonkeyPatch):
        ...
        host = static_worker_host(RELEASE, specs_rollout.ROLLOUT_EXECUTOR_POOL_ID)
    ...
    def test_the_rollout_components_assemble_around_those_handles(self, monkeypatch: pytest.MonkeyPatch):
        ...
        executor_host = static_worker_host(RELEASE, specs_rollout.ROLLOUT_EXECUTOR_POOL_ID)
        controller_host = static_worker_host(RELEASE, specs_inference.INFERENCE_CONTROLLER_POOL_ID)
        apps = {
    ...
    def test_the_trainer_controller_answers_over_rpc_with_no_actor_behind_it(self, monkeypatch: pytest.MonkeyPatch):
        ...
        host = static_worker_host(RELEASE, specs_train.compute_trainer_controller_pool_id("actor"))
