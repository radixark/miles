...
class _FakeTrainerCell:
    ...
    async def execute(self, fn_name: str, *, timeout: float, info):
        ...
        return [
            _rank_report(1, updated=tuple(info.engine_cell_ids)),
            _rank_report(1, updated=tuple(info.engine_cell_ids)),
        ]
...
class TestUpdateWeightsUsesEveryAliveCell:
    ...
    async def test_each_cell_reports_its_own_model_version(self):
        """The controller forwards trained model versions instead of allocating ordinals."""
        cells = [
            _FakeTrainerCell(0, outcome=[_rank_report(42, updated=("engine-0",))]),
            _FakeTrainerCell(1, outcome=[_rank_report(42, updated=("engine-1",))]),
        ]
        controller = _make_fanout_controller(cells)

        report = await controller.update_weights(info=_p2p_info(2))

        assert report.weight_version == 42
        assert all("weight_version" not in call for cell in cells for call in cell.calls)
    ...
    async def test_the_cells_send_at_the_same_time(self):
        ...
        class _BlockingCell(_FakeTrainerCell):
            async def execute(self, fn_name: str, *, timeout: float, info):
                started.set()
                await blocker.wait()
                return await super().execute(fn_name, timeout=timeout, info=info)
