...
class RankedWorker(FrozenStrictBaseModel):
    pod: pod_info.ParsedPod
    name: str
    rank_in_pod: int
    gpu_ids: list[int]


def compute_cell_info(cell_id: str, *, pods: list[pod_info.ParsedPod], run: KubernetesRun) -> CellInfo | None:
    if not pods:
        return None

    pool_id = pods[0].pool_id
    meta = _spec_meta_of_spec(pods[0], run=run) | _pod_meta_of_cell(cell_id, pods=pods)
    ...
    return CellInfo(
        ...
        workers_hash=pod_info.cell_members_hash(pods),
        meta=meta,
    )


def compute_worker_infos(
    cell_id: str, *, pods: list[pod_info.ParsedPod], run: KubernetesRun, worker_classes: WorkerClassLoader
) -> list[WorkerInfo]:
        ...
        infos.append(
            WorkerInfo(
                ...
                handle=build_rpc_handle(
                    worker_class=worker_classes.of_spec(worker.pod.pool_id),
                    addrs=addrs,
...
def workers_of_pods(pods: list[pod_info.ParsedPod], *, run: KubernetesRun) -> list[RankedWorker]:
    return [worker for pod in pods for worker in _ranks_of_pod(pod, run=run)]


def addrs_of_worker(worker: RankedWorker, *, run: KubernetesRun) -> NamedHostAndPorts:
    host = _host_of_pod(worker.pod, namespace=run.namespace)
    ports = run.pools[worker.pod.pool_id].ports
...
def _ranks_of_pod(pod: pod_info.ParsedPod, *, run: KubernetesRun) -> list[RankedWorker]:
    ranks_per_pod = run.pools[pod.pool_id].ranks_per_pod
    ...
    return [
        RankedWorker(
            ...
            name=compute_worker_name(
                cell_id=pod.cell_id, worker_in_cell_index=pod.pod_index * ranks_per_pod + rank_in_pod
            ),
...
def _host_of_pod(pod: pod_info.ParsedPod, *, namespace: str) -> str:
    if pod.pod_ip:
        return pod.pod_ip
...
def _has_every_pod(pods: list[pod_info.ParsedPod]) -> bool:
...
def _spec_meta_of_spec(pod: pod_info.ParsedPod, *, run: KubernetesRun) -> dict[str, Any]:
    compute_meta = run.pools[pod.pool_id].meta
    if compute_meta is None:
        return {}
    return dict(compute_meta(WorkerMetaContext(cell_id=pod.cell_id)))


def _pod_meta_of_cell(cell_id: str, *, pods: list[pod_info.ParsedPod]) -> dict[str, str]:
    merged: dict[str, str] = {}
    for pod in pods:
        for key, value in pod.meta.items():
