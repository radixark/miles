...
Please ensure that the converted checkpoint points to a directory where the `config.json` contains the correct `quantization_config` so that miles can automatically use FP8 quantization during weight updates.
...
Next, use the `tools/convert_hf_to_int4.py` script to convert BF16 weights to INT4 format. Ensure that the `--hf-checkpoint` parameter points to a directory where `config.json` contains the correct `quantization_config`. miles will automatically utilize INT4 quantization during weight updates.
