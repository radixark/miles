...
import pytest
from tests.fast.utils.workers.worker_provider.kubernetes.test_pod_info_parser import make_pod
...
from miles.utils.workers.backend_capability.kubernetes import compute_kubernetes_backend_capability
...
def engine_spec(*, num_cells: int = 2, gpu_offset: int = 0) -> CommandWorkerSpec:
    ...
    return CommandWorkerSpec(
        ...
        meta=lambda context: dict(
            ...
            gpu_offset=gpu_offset + context.cell_ordinal * GPUS_PER_NODE,
            sglang_api_key=None,
            needs_offload=False,
            update_weights=True,
        ),
...
def observed_meta(*, pool_id: str, cell_index: int, engine_cells: int = 2, trainer_cells: int = 1) -> dict[str, Any]:
    ...
    pods = [
        make_pod(
            name=f"{pool_id}-{cell_index}-{worker_index}",
            pool_id=pool_id,
    ...
    factory = compute_kubernetes_backend_capability(
        specs=specs,
        namespace=NAMESPACE,
        release=RELEASE,
        kubernetes_client_factory=lambda: FakePodApi(pods),
    )
    ...
    async def scenario() -> dict[str, Any]:
        ...
        try:
            info = provider.cell_info(f"{pool_id}-{cell_index}")
...
class TestWhatAnEngineCellReports:
    def test_carries_every_fact_the_inference_controller_indexes(self):
        ...
        meta = observed_meta(pool_id=ENGINE_POOL_ID, cell_index=0)
    ...
    def test_gives_each_cell_its_own_gpu_offset(self):
        ...
        first = observed_meta(pool_id=ENGINE_POOL_ID, cell_index=0)
        second = observed_meta(pool_id=ENGINE_POOL_ID, cell_index=1)
    ...
    def test_keeps_the_types_the_consumers_expect(self):
        ...
        meta = observed_meta(pool_id=ENGINE_POOL_ID, cell_index=1)
...
class TestWhatATrainerCellReports:
    def test_carries_every_fact_the_trainer_group_indexes(self):
        ...
        meta = observed_meta(pool_id=TRAINER_POOL_ID, cell_index=0)
    ...
    def test_carries_the_same_facts_for_every_cell_of_the_pool(self):
        ...
        first = observed_meta(pool_id=TRAINER_POOL_ID, cell_index=0, trainer_cells=2)
        second = observed_meta(pool_id=TRAINER_POOL_ID, cell_index=1, trainer_cells=2)
    ...
    def test_reports_the_role_the_spec_was_built_for(self):
        ...
        assert observed_meta(pool_id=TRAINER_POOL_ID, cell_index=0)["role"] == "actor"
