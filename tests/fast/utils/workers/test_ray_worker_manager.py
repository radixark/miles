...
@pytest.fixture
def recorded_gate_activations(monkeypatch) -> list[str]:
    ...
    monkeypatch.setattr(ray_worker_manager_mod, "activate_launch_gate", _activate)
    return activated


class TestLaunchGateActivation:
    def _gated_spec(self, name: str, **kwargs) -> CommandWorkerSpec:
        return _make_spec(
            name,
            port_infos=[
                PortInfo(name="primary", static_port=8000, allow_dynamic=True),
                PortInfo(name="gate", static_port=13000, mode="master", allow_dynamic=True),
            ],
            **kwargs,
        )

    async def test_every_gated_cell_is_released_once_its_workers_are_running(
        self, fake_ray_cluster: FakeRayCluster, recorded_gate_activations: list[str]
    ):
        """A gated engine idles at its gate until told to proceed, so each cell needs exactly one release."""
        manager = await _launch([self._gated_spec("engine", num_cells=2)])

        expected = [
            f"http://{manager.get_worker_addrs(f'engine-{cell_index}-0')['gate'].host}:"
            f"{manager.get_worker_addrs(f'engine-{cell_index}-0')['gate'].port}"
            for cell_index in range(2)
        ]
        assert sorted(recorded_gate_activations) == sorted(expected)

    async def test_the_release_targets_the_rank_zero_worker_of_a_multi_worker_cell(
        self, fake_ray_cluster: FakeRayCluster, recorded_gate_activations: list[str]
    ):
        """One cell shares one torch dist world, so only its rank-0 worker exposes the gate."""
        manager = await _launch([self._gated_spec("engine", num_workers_per_cell=3)])

        gate = manager.get_worker_addrs("engine-0-0")["gate"]
        assert recorded_gate_activations == [f"http://{gate.host}:{gate.port}"]

    async def test_a_spec_without_a_gate_port_is_never_activated(
        self, fake_ray_cluster: FakeRayCluster, recorded_gate_activations: list[str]
    ):
        """Routers and session servers run ungated, so the manager must not dial a port they lack."""
        await _launch([_make_spec("router")])

        assert recorded_gate_activations == []

    async def test_the_release_happens_only_after_every_command_was_issued(
        self, fake_ray_cluster: FakeRayCluster, monkeypatch
    ):
        """Releasing a gate before its engine is even running would dial a port nobody listens on yet."""
        import miles.utils.workers.ray_worker_manager as ray_worker_manager_mod

        runs_at_activation: list[int] = []

        async def _activate(gate_url: str) -> None:
            runs_at_activation.append(len(fake_ray_cluster.calls_of("run")))

        monkeypatch.setattr(ray_worker_manager_mod, "activate_launch_gate", _activate)
        await _launch([self._gated_spec("engine", num_cells=2)])

        assert runs_at_activation == [2, 2]
