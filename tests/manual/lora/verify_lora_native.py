...
class Args:
    ...
    hf_lora_targets = ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"]
    mla_hf_lora_targets = [
        "q_a_proj",
        "q_b_proj",
        "kv_a_proj_with_mqa",
...
def _write_config_only_checkpoint(model_type: str) -> str:
    """The registry resolves the spec from config.json; no weights are needed here."""
    import json
    import tempfile

    checkpoint = tempfile.mkdtemp(prefix="verify_lora_native_")
    with open(os.path.join(checkpoint, "config.json"), "w") as handle:
        json.dump({"model_type": model_type}, handle)
    return checkpoint
...
def main():
    ...
    if a.mla:
        Args.hf_lora_targets = Args.mla_hf_lora_targets
    Args.hf_checkpoint = _write_config_only_checkpoint("deepseek_v3" if a.mla else "qwen3")
