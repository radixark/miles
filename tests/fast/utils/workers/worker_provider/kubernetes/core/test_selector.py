...
from miles.utils.workers.worker_provider.kubernetes.helm import labels
...
class TestWatchedPodsSelector:
    def test_watches_nothing_when_no_pool_is_wanted(self) -> None:
        ...
        selector = provider._watched_pods_selector(
            base_selector=BASE_SELECTOR, pool_label_key=labels.DEFAULT_LABEL_KEYS.pool_id, pool_ids=[]
        )
    ...
    def test_scopes_the_watch_to_the_wanted_pools(self) -> None:
        ...
        selector = provider._watched_pods_selector(
            base_selector=BASE_SELECTOR, pool_label_key=labels.DEFAULT_LABEL_KEYS.pool_id, pool_ids=["engine"]
        )

        assert selector == f"{BASE_SELECTOR},{labels.DEFAULT_LABEL_KEYS.pool_id} in (engine)"
    ...
    def test_sorts_the_wanted_pools(self) -> None:
        ...
        selector = provider._watched_pods_selector(
            ...
            pool_label_key=labels.DEFAULT_LABEL_KEYS.pool_id,
            pool_ids=["rollout", "engine"],
        )

        assert selector == f"{BASE_SELECTOR},{labels.DEFAULT_LABEL_KEYS.pool_id} in (engine,rollout)"
