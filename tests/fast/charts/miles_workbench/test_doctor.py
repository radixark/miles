...
class TestDoctor:
    ...
    def fake_cluster(self, tmp_path, monkeypatch):
        ...
        escalate_path = tmp_path / "escalate"
        escalate_path.write_text("denied")
        ...
        (bin_dir / "kubectl").write_text(
            ...
            f'  if [ -s {escalate_path} ] && [ "$3" = "escalate" ]; then exit 1; fi\n'
    ...
    def test_escalate_and_bind_together_downgrade_a_missing_grant_to_a_warning(self, fake_cluster):
        ...
        (fake_cluster["denied_path"]).write_text("configmaps")
        (fake_cluster["escalate_path"]).write_text("")
    ...
    def test_a_bind_right_restricted_to_this_release_counts(self, fake_cluster):
        """Least-privilege clusters grant bind on one Role by name, which the unqualified question misses."""
        (fake_cluster["denied_path"]).write_text("configmaps bind:roles.rbac.authorization.k8s.io")
        (fake_cluster["escalate_path"]).write_text("")
        ...
        assert result.returncode == 0, result.stdout + result.stderr
        assert (
            "auth can-i bind roles.rbac.authorization.k8s.io/miles-workbench-alice -n rl"
    ...
    def test_a_grant_denied_without_escalate_is_a_failure(self, fake_cluster):
        ...
        (fake_cluster["denied_path"]).write_text("configmaps escalate")
    ...
    def test_missing_leaderworkerset_rights_name_the_admin_prerequisite(self, fake_cluster):
        ...
        (fake_cluster["denied_path"]).write_text(f"{LWS_RESOURCE} escalate")
