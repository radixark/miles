...
class TestStaleSubsystemLabelsCleared:
    def test_disappeared_subsystem_resets_to_monitoring(self) -> None:
        """When a subsystem disappears from subsystem_modes (e.g. during
        recovery where the main SM is not in NormalSt), its metrics must
        be reset to monitoring (is_recovery=False, phase=0) rather than
        retaining stale recovery labels."""
        registry, exporter = make_test_exporter()
        ...
        assert get_sample_value(registry, mn.CONTROLLER_MODE, labels={"subsystem": "training"}) == 1.0
        assert get_sample_value(registry, mn.CONTROLLER_MODE, labels={"subsystem": "rollout_0"}) == 1.0
        ...
        assert get_sample_value(registry, mn.CONTROLLER_MODE, labels={"subsystem": "training"}) == 0.0
        assert get_sample_value(registry, mn.CONTROLLER_RECOVERY_PHASE, labels={"subsystem": "training"}) == 0.0
        assert get_sample_value(registry, mn.CONTROLLER_MODE, labels={"subsystem": "rollout_0"}) == 0.0

    def test_subsystem_reappearing_after_disappearance(self) -> None:
        """A subsystem that disappears and reappears should reflect the new state."""
        registry, exporter = make_test_exporter()

        exporter.update_from_state(
            job_status=JobStatus.RUNNING,
            subsystem_modes={"training": (True, 3)},
            latest_loss=None,
            latest_mfu=None,
        )
        exporter.update_from_state(
        ...
        exporter.update_from_state(
            job_status=JobStatus.RUNNING,
            subsystem_modes={"training": (False, 0)},
            latest_loss=None,
            latest_mfu=None,
        )
