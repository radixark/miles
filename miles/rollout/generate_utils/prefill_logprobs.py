...
async def recompute_samples_rollout_logprobs_via_prefill(
    ...
    *,
    ...
    if _can_batch_prefill_score(args, samples_to_score):
        # SGLang can serve scoring requests from radix/KV cache if the generated
        # sequence is still resident. Flush once before the batched score pass so
        # this path is clean prefill, not a replay of decode-time logprobs.
