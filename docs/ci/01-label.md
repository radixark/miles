...
The `repo_write_access` group restricts `/clear-labels` and `/rerun-failed-ci` to live `write` or `admin` permission. Only `add_label_access` can contain explicit `user_ids`; those IDs do not grant any non-label operation.

`/rerun-test <test-file>` uses the `prior_contributor_access` group: the author association GitHub stamps on the comment admits `OWNER`, `MEMBER`, `COLLABORATOR`, and `CONTRIBUTOR` — anyone with a commit already merged into `radixark/miles` — without a live-permission lookup, and anyone else needs live `write` or `admin`. First-time contributors and users with no history in the repository are denied. This deliberately trades a weaker gate for self-serve targeted runs: a file run only ever executes a same-repository head that someone with write access already pushed, is bounded by one file and its registration's timeout, and is serialized per PR, so the residual exposure is runner time rather than code trust.
