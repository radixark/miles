---
title: GLM4.5
description: Launch recipe for GLM4.5 106B-A12B and 355B-A32B.
---
...
`$BASE_DIR` must be on a shared filesystem reachable from every node.
...
# Run on each node with NODE_RANK 0..1
...
??? tip "Fan-out from node 0"
    Append this loop at the end of the head launch script and the head will SSH out to
    workers:
...
To fit the 355 B variant on a single H200 node, see
