"""Phase-2 OpenEnv learning launcher (GLM-4.7-Flash, colocated single-node).

Same shape as ``run-openenv-echo.py`` but drives the OpenEnv Coding env
(``PythonCodeActEnv``) via ``openenv_coding_agent_function.run``. Unlike Echo, the
Coding env's safe-coding transform produces a *real* reward (penalizes dangerous
imports / syntax errors, rewards concise code), so this exercises GRPO learning
end to end through the seam: rollout/raw_reward should climb as the policy learns
to emit short, safe, valid Python.

Prereqs in the training image:
    pip install -e <OpenEnv>/envs/coding_env   # then re-pin: pip install 'openai==2.6.1'
    python make_coding_data.py --output /root/coding_train.jsonl --n 64
    python serve_coding_concurrent.py --port 8002 --max-concurrent 64   # colocated
...
class ScriptArgs(U.ExecuteTrainConfig):
    ...
    save_dir: str = "/workspace/GLM-4.7-Flash_openenv_coding/"
    prompt_data: str = "/root/coding_train.jsonl"

    # Training settings (small; learning signal but still a short run)
    ...
    # OpenEnv settings
    openenv_env_url: str = os.environ.get("OPENENV_ENV_URL", "http://localhost:8002")
    ...
    wandb_project: str = os.environ.get("WANDB_PROJECT", "openenv-coding-learn")
    wandb_team: str = os.environ.get("WANDB_TEAM", "")
    wandb_run_name: str = "openenv-coding-learn"

    # Prometheus settings
    use_prometheus: bool = True
    prometheus_port: int = 9090
    prometheus_run_name: str = "openenv-coding-learn"
...
def execute(args: ScriptArgs):
    ...
    agent_args = (
        ...
        "--custom-agent-function-path openenv_coding_agent_function.run "
