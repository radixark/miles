...
with more than one model-parallel rank. Explicit --exclude-prefix values mark
checkpoint-only modules disabled in the trainer, such as Lightning's mtp. head.
"""
...
def audit_export(bridge: AutoBridge, model: list, args: Args) -> None:
    ...
    with ExitStack() as stack, torch.no_grad(), patch_megatron_model(model):
        ...
        if rank == 0:
            excluded = {name for name in index if name.startswith(tuple(args.exclude_prefix))}
            missing = sorted(set(index) - seen - excluded)
            summary = {
