...
def convert_samples_to_train_data(
    ...
    inferred_row_counts = Counter(source_sample_indices)
    next_row_index: defaultdict[int, int] = defaultdict(int)
    sample_row_indices = []
    sample_row_counts = []
    for sample_index in source_sample_indices:
        sample = samples[len(sample_row_indices)]
        sample_row_indices.append(
            sample.sample_row_index if sample.sample_row_index is not None else next_row_index[sample_index]
        )
        sample_row_counts.append(
            sample.sample_row_count if sample.sample_row_count is not None else inferred_row_counts[sample_index]
        )
