...
// Torch host-shim launcher for Cake kernel 'kernel_cake_dsa_train_ad424e252c694e7e99a4' (host surface: cake_host_shim.h).
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_q, TensorView arg_kv, TensorView arg_do, TensorView arg_indices, TensorView arg_lse, TensorView arg_delta, TensorView arg_dq, TensorView arg_partial, int64_t arg_heads, int64_t arg_topk, int64_t arg_num_head_blocks, double arg_sm_scale, double arg_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_dsa_train_ad424e252c694e7e99a4),
  ...
  CAKE_HOST_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_dsa_train_ad424e252c694e7e99a4 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_dsa_train_ad424e252c694e7e99a4), kargs);
  CAKE_HOST_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_dsa_train_ad424e252c694e7e99a4 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_89e03cdcdad6ff0b

CAKE_HOST_EXPORT(run, cake_host_shim_89e03cdcdad6ff0b::Run);
