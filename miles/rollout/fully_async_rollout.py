...
class FullyAsyncRolloutFn(BaseRolloutFn):
    ...
    def __init__(self, input: RolloutFnConstructorInput):
        ...
        self._pending_put: DataBufferInput | None = None
    ...
    def _start_worker(self, *, weight_version: int | None) -> None:
        ...
        if (restore := self._pending_restore) is not None:
            num_restored = sum(len(entries) for entries in restore.values()) + int(self._pending_put is not None)
            assert (
                num_restored == 0 or weight_version is not None
            ), "restored rollout output requires a weight version before the producer starts"
    ...
    def _submit_one_group(self) -> asyncio.Task:
        ...
        task = asyncio.create_task(self._generate_group(pending.samples))
    ...
    async def _worker_loop(self):
        ...
        while True:
            ...
            for task in done:
                self._pending_put = task.result()
                self._in_flight.pop(task)
                await self._flush_pending_put()

    async def _flush_pending_put(self) -> None:
        if (entry := self._pending_put) is None:
            return
    ...
    def _recycle(self, prompt_group: list[Sample], reason: UnusedReason = UnusedReason.ABORTED) -> None:
        ...
        self._retry_buffer.append(_PendingPrompt(samples=prompt_group))
    ...
    def save(self, rollout_id: int) -> None:
        ...
        logger.info(f"Saved fully async rollout state to {path}")
    ...
    def load(self, rollout_id: int | None = None) -> None:
        ...
        if not path.exists():
            logger.warning(f"No fully async rollout state found at {path}")
            return
        state = torch.load(path, weights_only=False)
        self._retry_buffer.extend(state["retry_buffer"])
        self._retry_buffer.extend(state["in_flight"])
        self._pending_put = state["pending_put"]
        self._pending_restore = state["output"]
        logger.info(f"Loaded fully async rollout state from {path}")
    ...
    def _collect_state(self) -> dict[str, Any]:
        if self._pending_put is not None and self._pending_put.completed:
            self._pending_put = None
        output = {key: list(entries) for key, entries in self._in_transit.items()}
        if self._pending_restore is not None:
            buffered = self._pending_restore
        elif self._output is not None:
            buffered = self._output.snapshot()
        else:
            buffered = {}
        ...
        return {
            ...
            "in_flight": [
                _PendingPrompt(samples=_copy_reset_for_retry(pending.samples)) for pending in self._in_flight.values()
            ],
