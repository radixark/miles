...
the trainer takes one group at a time to assemble a training batch. The buffer decides what to
...
| `get(**context)` | The trainer, once per group | Return one entry, waiting until a group is available |
...
flowchart LR
    ...
    GF -->|"get()"| T["Trainer receives<br/>one group at a time"]
    U -->|retry| DS
