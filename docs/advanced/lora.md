...
| Bridge | Qwen3-8B on ROCm | Dense | [AMD launcher](https://github.com/radixark/miles/blob/main/scripts/amd/run_qwen3_8b_lora.py) | 8×MI350X. `fa3`/`fa4`/`flashinfer` have no HIP kernels, so the launcher pins the SGLang `triton` attention backend. KL vs the [reference logprobs](https://huggingface.co/datasets/yushengsu/lora-diff-Qwen3-8B) 2.82e-4 against a 5e-3 threshold. |
