"""Standalone Session Middleware proxy.

A lightweight FastAPI application that sits in front of an sglang router,
intercepts session-related requests (CRUD + chat completions with TITO
pretokenized prefix reuse), and transparently forwards everything else.

Usage (CLI):
    python -m miles.session_middleware.server \\
        --host 0.0.0.0 --port 8000 \\
        --upstream-url http://localhost:30000 \\
        --hf-checkpoint Qwen/Qwen3-0.6B

Environment variables:
    SESSION_MIDDLEWARE_TIMEOUT  – HTTP proxy timeout in seconds (default: no timeout)
"""
...
def create_session_middleware_app(args) -> FastAPI:
    ...
    timeout_env = os.environ.get("SESSION_MIDDLEWARE_TIMEOUT")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Session Middleware – standalone proxy for sglang router")
    ...
    app = create_session_middleware_app(ns)
